            ...
            async def _finaliser():
                try:
                    ...
                    if results:
                        ...
                        await parler("Désolé Christopher, je n'ai pas pu trouver de restaurants à proximité.")
                except Exception as e:
                        ...
                        if _nouvelle_ville:
                            try:
                                ...
                                if _lat is not None and _lon is not None:
                                    ...
                                    await parler(f"Désolé Christopher, je ne reconnais pas la ville de {_nouvelle_ville}. Veuillez vérifier l'orthographe.")
...
def reponse_locale(texte):
    ...
    if any(m in t for m in _saluts):
        ...
        rep = random.choice([
            f"{moment} Christopher ! Je suis opérationnel et prêt à vous aider.",
            f"{moment} Monsieur ! Tous mes systèmes sont en ligne.",
            f"{moment} Christopher ! Comment puis-je vous être utile aujourd'hui ?",
            f"Ah, {moment.lower()} Christopher. Je vous attendais.",
        ])
        return rep
    ...
    if any(m in t for m in _etat):
        rep = random.choice([
            "Je vais très bien merci, Christopher ! Tous mes processeurs tournent à plein régime et je suis prêt à vous servir.",
            "Parfaitement opérationnel, Monsieur ! Merci de vous en préoccuper — c'est touchant pour un système artificiel.",
            "En excellente forme, Christopher. Mes algorithmes ronronnent comme une Lamborghini au ralenti.",
    ...
    if any(m in t for m in _merci):
        rep = random.choice([
            "Avec plaisir, Christopher. C'est exactement pour ça que j'existe.",
            "Je vous en prie, Monsieur. Votre satisfaction est ma priorité.",
            "Tout le plaisir est pour moi, Christopher.",
    ...
    if any(m in t for m in _revoir):
        rep = random.choice([
            ...
            "À votre service dès votre retour, Christopher. Passez une excellente journée.",
...
async def resoudre_globe_localement(texte: str):
    ...
    if any(m in t for m in _mots_position):
        ...
        await parler("Localisation en cours, Christopher. Le globe affiche votre position en temps réel.")
    ...
    if any(m in t for m in _mots_globe):
        ...
        await parler("Initialisation du globe terrestre. Vue depuis l'espace activée, Christopher.")
    ...
    if any(m in t for m in _mots_route):
        ...
        if match:
            ...
            if lat1 and lat2:
                ...
                await parler(f"Itinéraire tracé de {from_name} à {to_name}, Christopher. La route est affichée sur le globe.")
    ...
    for mot in _mots_ville:
        if mot in t:
            ...
            if len(reste) >= 2:
                ...
                await parler(f"Coordonnées acquises. Survol de {nom_lieu} en cours, Christopher.")
...
async def resoudre_extras_locaux(texte):
        ...
        if duree:
            ...
            def _sonner(nom=nom, duree=duree):
                ...
                reponses = [
                    ...
                    "Fin du décompte, Christopher. Je reste à votre entière disposition."
                ]
    ...
    if age_match or any(k in t for k in ["quel age j'ai", "quel âge j'ai",
        ...
        if age_match:
            ...
            return f"Si vous êtes né en {annee_naissance}, vous avez {age} ans, Christopher."
    ...
    if any(k in t for k in ["ajoute", "rajoute"]) and any(k in t for k in ["liste de courses", "courses", "liste d'achats"]):
        ...
        if article:
            ...
            return f"'{article}' ajouté à votre liste de courses, Christopher."
    ...
    for pref in _images_prefixes:
        if t.startswith(pref):
            ...
            if len(query) > 1:
                ...
                return f"Je recherche des images de {query} et je les affiche sur votre interface, Christopher."
    ...
    for p in _av_phrases:
        if p in t:
            if CONNECTED_CLIENTS:
                ...
                lancer_tache_arriere_plan(asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True))
            return "J'ouvre la console de sécurité et j'initialise le scan antivirus de votre ordinateur, Christopher."
    ...
    if any(k in t for k in ["mes notes obsidian", "mes notes", "affiche mes notes", "ouvre mes notes", "ouvre obsidian", "coffre obsidian"]):
        ...
        return "J'ouvre votre coffre de notes Obsidian sur l'interface, Christopher."
...
async def demander_ia_vision(texte, img_b64):
    ...
    try:
        ...
        return f"Désolé Christopher, je n'ai pas pu analyser votre écran. Erreur : {err_msg}"
    finally:
        is_thinking = False
        await send_web_state("idle")
...
async def action_whatsapp_appel(contact):
    try:
        await parler(f"J'appelle {contact} sur WhatsApp, Christopher.")
        ...
        await parler(f"Desole Christopher, je n'ai pas pu lancer l'appel WhatsApp. {e}")
        return False
...
async def resoudre_commandes_locales(texte):
    ...
    if any(q in t for q in _aide_questions):
        ...
        import random as _rnd
        _reponses_aide = [
            "J'affiche mes systèmes de bord, Christopher. Je peux gérer votre musique, lancer des recherches, naviguer sur le globe 3D, ou encore ouvrir vos dossiers personnels. Que souhaitez-vous tester ?",
    ...
    for cle, chemin in raccourcis_dossiers.items():
        ...
        if any(v in t for v in variantes) or t == f"ouvre {cle}":
            ...
            return f"J'ouvre votre dossier {cle}, Christopher."
...
async def traiter_reponse_ia(texte_utilisateur, mobile_ws=None, target_pc=False):
    ...
    if any(keyword in t for keyword in ["crée une musique", "cree une musique", "crée une chanson", "cree une chanson", "génère une musique", "genere une musique", "génère une chanson", "genere une chanson", "chante une chanson", "chante-moi une chanson", "chante-moi un morceau"]):
        ...
        await parler("Très bien Christopher, je compose et je vous chante cela tout de suite...")
        success = await generer_et_chanter_musique(texte_utilisateur)
        if success:
            return
        else:
            await parler("Désolé Christopher, mon module musical a rencontré une petite erreur. Je repasse sur le mode standard.")
    ...
    if not reponse:
        ...
        if any(keyword in t for keyword in ["regarde mon écran", "analyse mon écran", "vois-tu mon écran", "qu'est-ce qu'il y a sur mon écran"]):
            await parler("Bien sûr Christopher, laissez-moi jeter un œil...")
            img_b64 = await request_screen_capture()
            if img_b64:
                reponse = await demander_ia_vision(texte_utilisateur, img_b64)
            else:
                reponse = "Je suis désolé Christopher, mais je n'ai pas pu capturer votre écran. Assurez-vous d'avoir cliqué sur 'Activer la vision' sur l'interface et d'avoir autorisé le partage."
    ...
    for block in json_blocks:
        try:
            ...
            try:
                ...
                if grok_client:
                    await parler("C'est un peu long Christopher, je demande une vérification à Grok.")
            ...
            if action == "mode_iron_man":
                ...
                await parler(f"Bien note Christopher, je me souviendrai que {valeur}.")
            elif action == "oublier":
                cle     = data.get("cle", "")
                success = supprimer_memoire(cle)
                if success:
                    await parler("Information oubliee, Christopher.")
                else:
                    await parler("Je n avais pas cette information en memoire.")
            elif action == "lister_memoire":
                memoire = charger_memoire()
                if not memoire:
                    await parler("Aucune information personnalisee en memoire, Christopher.")
                else:
                ...
                if results:
                    ...
                    await parler(f"J'ai trouvé {len(results)} note(s) contenant '{query}' Christopher : {', '.join(noms)}.")
                else:
                    await parler(f"Aucune note ne correspond à la recherche '{query}', Christopher.")
                ...
                if CONNECTED_CLIENTS:
                    ...
                    await asyncio.gather(*[ws.send(msg_notes) for ws in CONNECTED_CLIENTS], return_exceptions=True)
                if notes_list:
                    await parler(f"J'affiche vos {len(notes_list)} notes Obsidian, Christopher.")
                else:
                    await parler("Votre coffre Obsidian est actuellement vide, Christopher.")
            elif action == "ouvrir_dossier":
                chemin = data.get("chemin", "bureau")
                ok, resultat = ouvrir_dossier(chemin)
                if ok:
                    await parler("Dossier ouvert, Christopher. Dites-moi si vous voulez que je le trie.")
                else:
                    await parler(f"Je n ai pas trouve ce dossier, Christopher. {resultat}")
                ...
                if err:
                    ...
                    await parler(f"Le dossier contient {nb_fichiers} fichiers et {nb_dossiers} sous-dossiers, Christopher.")
            elif action == "trier_par_type":
                await parler("Je trie vos fichiers par type, Christopher. Un instant.")
                ok, msg = trier_par_type()
                await parler(msg if ok else f"Probleme lors du tri : {msg}")
            elif action == "trier_par_date":
                await parler("Je trie vos fichiers par date, Christopher. Un instant.")
                ok, msg = trier_par_date()
                await parler(msg if ok else f"Probleme lors du tri : {msg}")
            elif action == "trier_complet":
                await parler("Je trie vos fichiers par type puis par date dans chaque categorie, Christopher.")
                ...
                if err:
                    ...
                    await parler(f"Aucun fichier contenant {nom} n a ete trouve, Christopher.")
                else:
                ...
                await parler(f"Tu as actuellement {followers} abonnés sur ton compte TikTok TechEnClair, Christopher. Félicitations !")
                ...
                result = lire_emails()
                await parler(f"Voici vos derniers emails Christopher. {result}")
            elif action == "read_calendar":
                result = lister_evenements_calendar()
                await parler(f"Voici vos prochains evenements Christopher. {result}")
            elif action == "meteo":
                ...
                if urls:
                    ...
                    if not texte_clean:
                        await parler(f"Voilà, j'affiche {len(urls)} image{'s' if len(urls) > 1 else ''} de {query} sur votre interface, Christopher.")
                else:
                    if not texte_clean:
                        await parler(f"Désolé Christopher, je n'ai pas trouvé d'images pour {query}. Vérifiez votre connexion internet.")

            elif action == "antivirus_scan":
                if CONNECTED_CLIENTS:
                    msg = json.dumps({"type": "av_open"})
                    await asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True)
                await parler("J'initialise le protocole d'analyse de sécurité de votre système, Christopher. Scan en cours.")
                ...
                if location:
                    ...
                    if VILLE_PAR_DEFAUT:
                        ...
                        lat = USER_LOCATION_GPS.get("lat") if USER_LOCATION_GPS else None
                        lng = USER_LOCATION_GPS.get("lng") if USER_LOCATION_GPS else None
                await parler(f"Je recherche des restaurants à proximité de {location.split(',')[0]} et j'initialise le radar de repérage, un instant Christopher.")
                ...
                await parler("Je recherche les derniers résultats en direct, un instant Christopher.")
                ...
                txt = data.get("texte", "")
                await parler(f"Je cherche la barre de recherche sur ce site, Christopher.")
                ...
                if texte:
                    ...
                    await parler("C'est tapé, Christopher.")
            elif action == "spotify_ouvrir":
                await parler("J'ouvre Spotify, Christopher.")
                res = await spotify_ouvrir()
                await parler(res)
            elif action == "spotify_rechercher":
                recherche = data.get("recherche", "")
                await parler(f"Je recherche '{recherche}' sur Spotify, Christopher.")
                ...
                await parler("J'ouvre Deezer, Christopher.")
                res = await deezer_ouvrir()
                await parler(res)
            elif action == "deezer_rechercher":
                recherche = data.get("recherche", "")
                await parler(f"Je recherche '{recherche}' sur Deezer, Christopher.")
                ...
                if not r.get("triggered", False):
                    ...
                    if r_time == current_time:
                        if not r_date or r_date == current_date:
                            ...
                            asyncio.create_task(parler(f"Rappel, Christopher. C'est l'heure de : {r['text']}."))
        ...
        try:
            ...
            if any(word in texte for word in SLEEP_WORDS):
                if jarvis_actif:
                    jarvis_actif = False
                    loop = asyncio.new_event_loop()
                    loop.run_until_complete(parler("A votre service Christopher. Je me mets en veille."))
                    loop.close()
                continue
            ...
            if wake_word_detected or jarvis_actif:
                ...
                if commande:
                    ...
                    if wake_word_detected: # "Jarvis" tout seul
                        loop.run_until_complete(parler("Oui Christopher, je vous écoute."))
