...
def _charger_user_name():
    ...
    try:
        ...
        with open(_p, "r", encoding="utf-8") as _f:
            return _j.load(_f).get("user_name", "Mickael")
    except Exception:
        return "Mickael"
...
STOP_PARLER        = False
MIC_MUTED          = False
ALWAYS_LISTEN_MODE = False  # Mode écoute permanente — bypass du mot-clé d'activation
_last_jarvis_speech = ""    # Dernière phrase dite par JARVIS (pour détection d'écho)
MIC_NEED_RELOAD = False
MIC_FORCED_INDEX = None  # Index imposé explicitement par l'christopher via les paramètres
...
def _nettoyer_emojis_pour_tts(texte: str) -> str:
...
def _effet_visuel_iron_man(stop_event):
    ...
    logs_fictifs = [
        ...
        "[COMPILE] Generating function signature: def executer(texte_christopher=None)",
...
def jarvis_creer_competence(nom_competence: str, description_demande: str) -> str:
    ...
    try:
        system_instruction = (
            ...
            "3. Tu dois obligatoirement implémenter une fonction principale nommée `executer(texte_christopher=None)` qui prend un argument optionnel (string) et qui RETOURNE obligatoirement une string (le texte formaté que JARVIS lira à voix haute).\n"
        ...
        return f"Désolé Mickael, la génération de la compétence a échoué. Détail de l'erreur : {e}"
...
def executer_competence_vocale(nom_competence: str, texte_recu: str = None) -> str:
    ...
    if not os.path.exists(filename):
        return f"Désolé Mickael, la compétence '{nom_competence}' n'est pas disponible."
...
async def generer_et_chanter_musique(prompt_christopher: str) -> bool:
    ...
    try:
        ...
        music_prompt = (
            f"Chante une courte chanson (maximum 20 secondes) répondant à la demande : {prompt_christopher}. "
...
async def parler(texte):
    ...
    # Remplacement du nom d'christopher
    texte = texte.replace("Mickael", USER_NAME).replace("mickael", USER_NAME.lower())
    ...
    global is_speaking, STOP_PARLER, speak_volume, _last_jarvis_speech
    STOP_PARLER = False
    is_speaking = True
    _last_jarvis_speech = texte  # Enregistre ce que JARVIS va dire pour la détection d'écho
    ...
    async def pre_generate(index):
        ...
        phrase_tts = _nettoyer_emojis_pour_tts(phrase_tts)
        if not phrase_tts:
            return None
        ...
        try:
            # ── OpenAI TTS (en ligne, nécessite OPENAI_API_KEY) ───────────────
            if voix_choisie.startswith("openai_"):
                from openai_tts import openai_tts_to_file
                ok = await asyncio.to_thread(openai_tts_to_file, phrase_tts, voix_choisie, tmp_file)
                if ok:
                    return tmp_file
                # Aucune clé, quota ou erreur réseau : priorité au modèle local Kokoro.
                tmp_wav = tmp_file.replace(".mp3", ".wav")
                created_files.append(tmp_wav)
                if await _kokoro_tts_to_file(phrase_tts, "kokoro_ff_siwis", tmp_wav):
                    print("[OPENAI TTS] Bascule automatique sur Kokoro local (Siwis).")
                    return tmp_wav
                # Dernier secours universel : Edge TTS Henri.
                fallback_voice = VOICE_MAP.get("male", "fr-FR-HenriNeural")
                communicate = edge_tts.Communicate(phrase_tts, voice=fallback_voice)
                await communicate.save(tmp_file)
                return tmp_file

            # ── ElevenLabs TTS (Ultra-Réaliste, multilingual) ─────────────────
            if voix_choisie.startswith("elevenlabs_"):
                # L'ID de voix ElevenLabs est stocké après le préfixe "elevenlabs_"
                el_voice_id = voix_choisie[len("elevenlabs_"):]
                el_api_key  = os.getenv("ELEVENLABS_API_KEY", "").strip()
                if el_voice_id and el_api_key:
                    ...
                    ok = await asyncio.to_thread(
                        ...
                        phrase_tts, el_voice_id, el_api_key, tmp_file
                    )
                    if ok:
                        return tmp_file
                else:
                    print("[ELEVENLABS TTS] Clé API ou voice_id manquant — fallback Edge TTS")
                # Fallback Edge TTS si ElevenLabs échoue ou clé manquante
                fallback_voice = VOICE_MAP.get("male", "fr-FR-HenriNeural")
                communicate = edge_tts.Communicate(phrase_tts, voice=fallback_voice)
                await communicate.save(tmp_file)
                return tmp_file

            # ── Piper TTS (100% local, optimisé pour la vitesse CPU) ──────────
            if voix_choisie.startswith("piper_"):
                from local_tts_engines import piper_tts_to_file
                tmp_wav = tmp_file.replace(".mp3", ".wav")
                created_files.append(tmp_wav)
                ok = await asyncio.to_thread(piper_tts_to_file, phrase_tts, voix_choisie, tmp_wav)
                if ok:
                    return tmp_wav
                fallback_voice = VOICE_MAP.get("male", "fr-FR-HenriNeural")
                communicate = edge_tts.Communicate(phrase_tts, voice=fallback_voice)
            ...
            # ── Chatterbox (100% local, haute qualité, GPU recommandé) ────────
            if voix_choisie.startswith("chatterbox_"):
                from local_tts_engines import chatterbox_tts_to_file
                tmp_wav = tmp_file.replace(".mp3", ".wav")
                created_files.append(tmp_wav)

                def _chatterbox_progress(percent, status):
                    send_web_broadcast_sync({
                        "type": "chatterbox_generation_progress",
                        "percent": percent,
                        "status": status,
                    })

                ok = await asyncio.to_thread(
                    chatterbox_tts_to_file, phrase_tts, tmp_wav, _chatterbox_progress
                )
                if ok:
                    return tmp_wav
                fallback_voice = VOICE_MAP.get("male", "fr-FR-HenriNeural")
            ...
            # ── Voix Kokoro-TTS Local (100% Local & Studio) ───────────────────
            if voix_choisie.startswith("kokoro_"):
                tmp_wav = tmp_file.replace(".mp3", ".wav")
                created_files.append(tmp_wav)
                ok = await _kokoro_tts_to_file(phrase_tts, voix_choisie, tmp_wav)
                if ok:
                    return tmp_wav
            ...
            if voix_choisie in _GEMINI_LIVE_VOICE_MAP:
                ...
                if ok:
                    return tmp_wav
                # Fallback Edge TTS si Live échoue (Voix 100% Française sans saut d'accent)
    ...
    try:
        ...
        for index, phrase in enumerate(phrases):
            ...
            if not current_file:
                # Si l'audio a échoué à se générer, on affiche le texte et on attend un peu pour que l'christopher lise
            ...
            if _skip_pc_audio:
                ...
                # Estimer la durée de lecture sur mobile et attendre avec vérification STOP_PARLER
                duration = max(1.5, len(phrase.split()) * 0.38)
                t_mob = time.time()
                while time.time() - t_mob < duration:
                    if STOP_PARLER:
                        if CONNECTED_CLIENTS:
                            try:
                                await asyncio.gather(*[ws.send(json.dumps({"action": "stop_audio"})) for ws in CONNECTED_CLIENTS])
                            except Exception:
                                pass
                        break
                    await asyncio.sleep(0.05)
            ...
            async def _finaliser():
                try:
                    ...
                    if results:
                        ...
                        await parler("Désolé Mickael, je n'ai pas pu trouver de restaurants à proximité.")
                except Exception as e:
...
ELEVENLABS_VOICE_ID  = os.getenv("ELEVENLABS_VOICE_ID", "")  # ID de la voix ElevenLabs choisie
...
def recharger_clients_ia():
    global GEMINI_API_KEY, YOUTUBE_API_KEY, XAI_API_KEY, SERPAPI_API_KEY, GROQ_API_KEY, ANTHROPIC_API_KEY, MISTRAL_API_KEY, OPENAI_API_KEY, DEEPSEEK_API_KEY
    global ELEVENLABS_API_KEY, ELEVENLABS_VOICE_ID
    global gemini_actif, client, grok_client, groq_client, anthropic_client, mistral_client, openai_client, deepseek_client
    ...
    deepseek_enabled  = cfg.get("api_deepseek_enabled", True)
    ...
    deepseek_key_to_use = DEEPSEEK_API_KEY if deepseek_enabled else ""
    ...
    if _cle_valide(deepseek_key_to_use):
        deepseek_client = OpenAI(api_key=deepseek_key_to_use, base_url="https://api.deepseek.com")
    else:
        deepseek_client = None

    # ── ElevenLabs TTS — rechargement à chaud ────────────────────────────────
    if _cle_valide(ELEVENLABS_API_KEY):
        print(f"[ELEVENLABS] Clé API chargée — voix ID actuelle : '{ELEVENLABS_VOICE_ID or 'non définie'}'")
    else:
        print("[ELEVENLABS] Aucune clé API configurée — ElevenLabs TTS désactivé.")
...
def _install_jarvis_os(path, websocket, loop):
    ...
    try:
        ...
        if run_proc.returncode == 0:
            send_progress("Installation terminée avec succès.", 100, "Le conteneur est prêt.", True, 3000)
        else:
...
async def ws_handler(websocket):
    global interface_deja_connectee, STOP_PARLER, MIC_MUTED, ALWAYS_LISTEN_MODE, USER_LOCATION_GPS, NEMOTRON_ASR_ENABLED, _nemotron_instance, USER_NAME, USER_AGE, MUSIQUE_LIEN_PERSO, MIC_NEED_RELOAD, jarvis_actif, VILLE_PAR_DEFAUT, LAT_PAR_DEFAUT, LON_PAR_DEFAUT, ATTENTE_CHOIX_MODELE_IMAGE, ATTENTE_CHOIX_MODELE_SITE, PROMPT_EN_ATTENTE, CHOSEN_MODELS, ATTENTE_CREATION_PROMPT
    ...
    # Push du statut Gmail configuré
    try:
        from google_services import gmail_est_configure
        if gmail_est_configure():
            await websocket.send(json.dumps({"type": "gmail_status", "configured": True}))
        await websocket.send(json.dumps({"type": "always_listen_state", "active": ALWAYS_LISTEN_MODE}))
        ...
        async for message in websocket:
            try:
                ...
                if data.get("type") == "mobile_command":
                    ...
                    if deepseek_client: models.append("deepseek")
                    ...
                    status_data = _verifier_ollama_statut()
                    await websocket.send(json.dumps({
                    ...
                    loop = asyncio.get_running_loop()
                    threading.Thread(target=_pull_ollama_model, args=(model_name, loop, websocket), daemon=True).start()
                    ...
                    loop = asyncio.get_running_loop()
                    threading.Thread(target=_installer_kokoro_tts, args=(loop, websocket), daemon=True).start()
                elif data.get("type") == "check_local_tts_status":
                    from local_tts_engines import get_local_tts_status
                    await websocket.send(json.dumps({
                        "type": "local_tts_status_reply",
                        "status": get_local_tts_status()
                    }))
                elif data.get("type") == "install_local_tts":
                    engine = str(data.get("engine", "")).lower()
                    if engine in {"piper", "chatterbox"}:
                        loop = asyncio.get_running_loop()
                        requested_voice = str(data.get("voice", "piper_fr_FR-siwis-medium"))

                        def _install_optional_tts_worker(selected_engine, selected_voice):
                            try:
                                from local_tts_engines import install_local_tts_engine
                                asyncio.run_coroutine_threadsafe(websocket.send(json.dumps({
                                    "type": "local_tts_install_progress",
                                    "engine": selected_engine,
                                    "status": f"Installation de {selected_engine.title()} en cours... Cela peut prendre plusieurs minutes.",
                                    "done": False
                                })), loop)
                                success, message = install_local_tts_engine(selected_engine)
                                if success and selected_engine == "piper":
                                    if selected_voice.startswith("piper_"):
                                        from local_tts_engines import ensure_piper_voice
                                        success, message = ensure_piper_voice(selected_voice)
                            except BaseException as exc:
                                success = False
                                message = f"Installation interrompue : {exc}"
                            asyncio.run_coroutine_threadsafe(websocket.send(json.dumps({
                                "type": "local_tts_install_progress",
                                "engine": selected_engine,
                                "status": message,
                        ...
                        threading.Thread(
                            target=_install_optional_tts_worker,
                            args=(engine, requested_voice),
                            daemon=True
                        ).start()
                ...
                elif data.get("type") == "get_windy_webcams":
                    bbox = data.get("bbox", {})
                    city_query = data.get("city_query", "").strip()
                    north = bbox.get("north", 51.97)
                    east = bbox.get("east", 6.77)
                    south = bbox.get("south", 43.97)
                    west = bbox.get("west", -1.23)
                    limit = bbox.get("limit", 45)
                    
                    def _fetch_webcams_worker(n, e, s, w, lim, query_text):
                        try:
                            from dotenv import load_dotenv
                            load_dotenv(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env"), override=True)
                            key = os.getenv("WINDY_API_KEY", "").strip()
                            if not key or key == "Votre ID" or key == "VOTRE_CLE_ICI" or "VOTRE" in key.upper():
                                print("[WINDY] Aucune clé API Windy valide configurée. Veuillez la renseigner dans Menu > Configuration API.")
                                send_web_broadcast_sync({
                                    "action": "windy_webcams_result",
                                    "webcams": [],
                            ...
                            if query_text:
                                # Recherche de coordonnées pour la ville demandée
                                try:
                                    geo_url = f"https://nominatim.openstreetmap.org/search?q={requests.utils.quote(query_text)}&format=json&limit=1"
                                    geo_resp = requests.get(geo_url, headers={"User-Agent": "JARVIS-HUD/2.6"}, timeout=5)
                                    if geo_resp.status_code == 200 and geo_resp.json():
                                        first = geo_resp.json()[0]
                                        c_lat = float(first["lat"])
                                        c_lon = float(first["lon"])
                                        span = 3.5
                                        n = min(85.0, c_lat + span)
                                        s = max(-85.0, c_lat - span)
                                        e = min(180.0, c_lon + span)
                                        w = max(-180.0, c_lon - span)
                                        # Repositionner aussi le globe 3D sur cette ville
                                        send_web_broadcast_sync({
                                            "action": "jarvis_globe",
                                            "globe_action": "fly_to",
                                            "lat": c_lat,
                                    ...
                                    print(f"[WINDY] Erreur géocodage ville : {geo_err}")

                            url = f"https://api.windy.com/webcams/api/v3/webcams?bbox={n},{e},{s},{w}&limit={lim}&include=images,location,player,urls"
                            resp = requests.get(url, headers={"x-windy-api-key": key, "Accept": "application/json"}, timeout=8)
                            if resp.status_code == 200:
                                res_data = resp.json()
                                cams = res_data.get("webcams", [])
                                print(f"[WINDY] {len(cams)} webcams récupérées avec succès (Player & URLs inclus).")
                                send_web_broadcast_sync({
                                    "action": "windy_webcams_result",
                                    "webcams": cams
                                })
                            else:
                                print(f"[WINDY] Erreur API : {resp.status_code} {resp.text[:100]}")
                                send_web_broadcast_sync({
                                    "action": "windy_webcams_result",
                                    "webcams": []
                                })
                        except Exception as exc:
                            print(f"[WINDY] Erreur fetch backend : {exc}")
                            send_web_broadcast_sync({
                                "action": "windy_webcams_result",
                                "webcams": []
                            })
                    
                    threading.Thread(target=_fetch_webcams_worker, args=(north, east, south, west, limit, city_query), daemon=True).start()

                # ── ElevenLabs : récupération des voix disponibles ─────────────
                elif data.get("type") == "get_elevenlabs_voices":
                    def _fetch_elevenlabs_voices_worker():
                        try:
                            from elevenlabs_tts import get_elevenlabs_voices
                            key = os.getenv("ELEVENLABS_API_KEY", "").strip()
                            voices = get_elevenlabs_voices(key) if key else []
                            send_web_broadcast_sync({
                                "type": "elevenlabs_voices_result",
                                "voices": voices
                            })
                        except Exception as _ev_err:
                            print(f"[ELEVENLABS WS] Erreur récupération voix : {_ev_err}")
                            send_web_broadcast_sync({
                                "type": "elevenlabs_voices_result",
                                "voices": [],
                                "error": str(_ev_err)
                            })
                    threading.Thread(target=_fetch_elevenlabs_voices_worker, daemon=True).start()

                # ── Wallpaper : Récupération du fond d'écran Windows ────────────
                elif data.get("type") == "get_desktop_wallpaper":
                    def _fetch_desktop_wallpaper_worker():
                        try:
                            import base64
                            import winreg
                            wp_path = None
                            appdata = os.getenv("APPDATA", "")
                            cand = os.path.join(appdata, "Microsoft", "Windows", "Themes", "TranscodedWallpaper")
                            ...
                            if wp_path and os.path.exists(wp_path):
                                with open(wp_path, "rb") as img_f:
                                    b64 = base64.b64encode(img_f.read()).decode("utf-8")
                                data_url = f"data:image/jpeg;base64,{b64}"
                                send_web_broadcast_sync({
                                    "type": "desktop_wallpaper_result",
                                    "data_url": data_url
                                })
                            else:
                                send_web_broadcast_sync({
                            ...
                            print(f"[WALLPAPER WS] Erreur récupération wallpaper bureau : {_wp_err}")
                            send_web_broadcast_sync({
                                "type": "desktop_wallpaper_result",
                                "data_url": None,
                                "error": str(_wp_err)
                            })
                    threading.Thread(target=_fetch_desktop_wallpaper_worker, daemon=True).start()

                elif data.get("type") == "read_single_email":

                    email_id = data.get("email_id")
                    email_index = data.get("index")
                    target = email_id if email_id is not None else email_index
                    detail = recuperer_email_detail(target)
                    if detail.get("success"):
                        await send_web_broadcast({"action": "display_email_detail_hud", "email": detail})
                        await parler(detail.get("vocal"))
                elif data.get("type") == "open_email_hud":
                    from google_services import recuperer_emails_structures
                    data_emails = recuperer_emails_structures(max_results=4)
                    emails_list = data_emails.get("emails", [])
                    await websocket.send(json.dumps({"action": "display_emails_hud", "emails": emails_list}))
                elif data.get("type") == "trash_email":
                    from google_services import supprimer_email
                    email_id = data.get("email_id")
                    email_index = data.get("index")
                    target = email_id if email_id is not None else email_index
                    res = supprimer_email(target)
                    await websocket.send(json.dumps({"action": "email_trashed", "id": email_id, "success": res.get("success")}))
                    await parler(res.get("vocal"))
                elif data.get("type") == "send_new_email":
                    from google_services import envoyer_email
                    to_addr = data.get("to", "")
                    subject = data.get("subject", "")
                    body    = data.get("body", "")
                    res = envoyer_email(destinataire=to_addr, sujet=subject, corps_texte=body)
                    await websocket.send(json.dumps({"action": "email_sent_status", "success": res.get("success")}))
                    await parler(res.get("vocal"))
                elif data.get("type") == "reply_email":
                    from google_services import repondre_email
                    email_id = data.get("email_id")
                    email_index = data.get("index")
                    body = data.get("body", "")
                    target = email_id if email_id is not None else email_index
                    res = repondre_email(target, body)
                    await websocket.send(json.dumps({"action": "email_sent_status", "success": res.get("success")}))
                    await parler(res.get("vocal"))
                elif data.get("type") == "open_jarvis_folder":
                ...
                elif data.get("type") == "toggle_always_listen":
                    ALWAYS_LISTEN_MODE = not ALWAYS_LISTEN_MODE
                    await websocket.send(json.dumps({"type": "always_listen_state", "active": ALWAYS_LISTEN_MODE}))
                    if ALWAYS_LISTEN_MODE:
                        jarvis_actif = True  # Active la session immédiatement
                        print("[JARVIS] ✅ Mode écoute permanente ACTIVÉ — Je suis à votre écoute en continu.")
                    else:
                        jarvis_actif = False  # Repasse en veille
                        await send_web_state("idle")
                        print("[JARVIS] 🔇 Mode écoute permanente DÉSACTIVÉ — Retour en mode mot-clé.")
                    ...
                    config_data["api_keys"] = {
                        ...
                        "WINDY_API_KEY":       os.getenv("WINDY_API_KEY", ""),
                        "ELEVENLABS_API_KEY":  os.getenv("ELEVENLABS_API_KEY", ""),   # ElevenLabs TTS
                        "ELEVENLABS_VOICE_ID": os.getenv("ELEVENLABS_VOICE_ID", ""),  # Voix ElevenLabs sauvegardée
                    }
                    ...
                    if force_refresh:
                        vpn.fetch_servers(force=True)
                    countries = vpn.get_countries()
                    await websocket.send(json.dumps({
                        "type": "vpn_countries",
                        "countries": countries,
                        "catalog": vpn.get_catalog_info()
                    }))
                elif data.get("type") == "games_get_library":
                    import games_manager
                    games = games_manager.get_cached_games()
                    await websocket.send(json.dumps({
                        "type": "games_library",
                        "games": games,
                        "folders": games_manager.get_folders(),
                        "needs_scan": not games_manager.has_cached_catalog()
                    }))
                elif data.get("type") == "games_scan":
                    import games_manager
                    games = await asyncio.to_thread(games_manager.scan_games)
                    await websocket.send(json.dumps({
                        "type": "games_library",
                        "games": games,
                        "folders": games_manager.get_folders()
                    }))
                elif data.get("type") == "games_launch":
                    import games_manager
                    result = await asyncio.to_thread(games_manager.launch_game, data.get("game_id", ""))
                    await websocket.send(json.dumps({"type": "games_launch_result", "result": result}))
                    if result.get("success"):
                        asyncio.create_task(parler(f"Je lance {result['game']['name']}, {USER_NAME}."))
                elif data.get("type") == "games_remove_folder":
                    import games_manager
                    games_manager.remove_folder(data.get("path", ""))
                    games = await asyncio.to_thread(games_manager.scan_games)
                    await websocket.send(json.dumps({
                        "type": "games_library", "games": games, "folders": games_manager.get_folders()
                    }))
                elif data.get("type") == "games_hide":
                    import games_manager
                    games_manager.hide_game(data.get("game_id", ""))
                    games = await asyncio.to_thread(games_manager.scan_games)
                    await websocket.send(json.dumps({
                        "type": "games_library", "games": games, "folders": games_manager.get_folders()
                    }))
                elif data.get("type") == "games_restore_hidden":
                    import games_manager
                    games_manager.restore_hidden_games()
                    games = await asyncio.to_thread(games_manager.scan_games)
                    await websocket.send(json.dumps({
                        "type": "games_library", "games": games, "folders": games_manager.get_folders()
                    }))
                elif data.get("type") == "games_pick_program":
                    _program_loop = asyncio.get_running_loop()
                    def _pick_game_program():
                        root = None
                        try:
                            import tkinter as tk
                            ...
                            executable = filedialog.askopenfilename(
                                title="Choisir le programme ou jeu à ajouter",
                                filetypes=[("Programmes Windows", "*.exe"), ("Tous les fichiers", "*.*")]
                            )
                            if not executable:
                                return
                            default_name = os.path.splitext(os.path.basename(executable))[0]
                            name = simpledialog.askstring("Nom du jeu", "Nom affiché dans JARVIS :",
                                                          initialvalue=default_name, parent=root) or default_name
                            cover = filedialog.askopenfilename(
                                title="Choisir une affiche (facultatif — Annuler pour utiliser l'icône)",
                                filetypes=[("Images", "*.jpg *.jpeg *.png *.webp *.ico"), ("Tous les fichiers", "*.*")]
                            )
                            games_manager.add_custom_game(executable, name, cover)
                            games = games_manager.scan_games()
                            asyncio.run_coroutine_threadsafe(websocket.send(json.dumps({
                                "type": "games_library", "games": games, "folders": games_manager.get_folders()
                            })), _program_loop)
                        except Exception as exc:
                            print(f"[JEUX] Ajout manuel impossible : {exc}")
                        finally:
                            if root is not None:
                                try: root.destroy()
                                except Exception: pass
                    threading.Thread(target=_pick_game_program, daemon=True).start()
                elif data.get("type") == "games_pick_folder":
                    _games_loop = asyncio.get_running_loop()
                    def _pick_games_folder():
                        root = None
                        try:
                            import tkinter as tk
                            from tkinter import filedialog
                            ...
                            root.withdraw()
                            root.attributes('-topmost', True)
                            folder = filedialog.askdirectory(title="Ajouter une bibliothèque de jeux à JARVIS")
                            if folder:
                                games_manager.add_folder(folder)
                                games = games_manager.scan_games()
                                asyncio.run_coroutine_threadsafe(websocket.send(json.dumps({
                                    "type": "games_library", "games": games,
                                    "folders": games_manager.get_folders()
                                })), _games_loop)
                        except Exception as exc:
                            print(f"[JEUX] Sélection du dossier impossible : {exc}")
                        finally:
                            if root is not None:
                                try: root.destroy()
                                except Exception: pass
                    threading.Thread(target=_pick_games_folder, daemon=True).start()
                    ...
                    async def async_connect_task(country_code):
                        ...
                        if result.get("success"):
                            ...
                            if result.get("error") != "Annulé par l'christopher.":
                    ...
                    # Le catalogue de jeux est une mémoire locale séparée du cache WebView2.
                    # Le bouton de nettoyage doit aussi remettre "Mes jeux" à zéro.
                    try:
                        import games_manager
                        if os.path.exists(games_manager.CONFIG_FILE):
                            os.remove(games_manager.CONFIG_FILE)
                        print("[CACHE] Bibliothèque de jeux locale réinitialisée.")
                    except Exception as _e:
                        print(f"[CACHE] Impossible de réinitialiser la bibliothèque de jeux : {_e}")

                    # Repondre succes au front-end
                    await websocket.send(json.dumps({
                        "type": "cache_cleared",
                        "success": True,
                        "message": "Cache et bibliothèque de jeux vidés — rechargement en cours."
                    }))
                    ...
                    async def _verifier_page_maj_vocal():
                        try:
                            ...
                            if site_version:
                                ...
                                if v_remote > v_local:
                                    await parler(f"J'ai détecté une mise à jour, la version {site_version} est disponible. Veuillez télécharger la mise à jour sur le site de TechEnClair.")
                                elif v_remote == v_local:
                                    await parler(f"Votre version {CURRENT_VERSION} de Jarvis est déjà à jour.")
                                else:
                                    await parler(f"Votre version {CURRENT_VERSION} de Jarvis est à jour.")
                            else:
                                await parler("Page de mise à jour ouverte sur le site de TechEnClair.")
                        except Exception as _e:
                            print(f"[UPDATE] Erreur vérification vocale : {_e}")
                            await parler("Page de mise à jour ouverte sur le site de TechEnClair.")
                    ...
                    try:
                        ...
                        print(f"[UPDATE] Erreur récupération lien manuel : {_e}")
                        _dl_url = "https://www.techenclair.fr/pages/jarvis"
                    await websocket.send(json.dumps({
                ...
                elif data.get("type") in ("ha_get_states", "ha_call_service", "ha_add_voice_alias"):
                    ...
                    if "user_name" in settings:
                        _ancien = config_data.get("user_name", "Mickael")
                        ...
                        if _nouvelle_ville:
                            try:
                                ...
                                if _lat is not None and _lon is not None:
                                    ...
                                    await parler(f"Désolé Mickael, je ne reconnais pas la ville de {_nouvelle_ville}. Veuillez vérifier l'orthographe.")
                    ...
                    global WAKE_WORD
                    USER_NAME = config_data.get("user_name", "Mickael")
                ...
                # ══════════════════════════════════════════════════════════
                # MODULE IMPRESSION 3D (Image → STL)
                # ══════════════════════════════════════════════════════════
                elif data.get("type") == "print3d_check_hardware":
                    try:
                        import image_to_stl
                        hw = image_to_stl.check_hardware()
                        await websocket.send(json.dumps({"type": "print3d_hardware_status", **hw}))
                    except Exception as e:
                        await websocket.send(json.dumps({"type": "print3d_hardware_status", "profile": "CPU_ONLY", "cuda_available": False, "gpu_name": None, "vram_total_gb": 0, "message": f"Erreur diagnostic : {e}"}))

                elif data.get("type") == "print3d_check_deps":
                    try:
                        import image_to_stl
                        deps = image_to_stl.check_dependencies()
                        await websocket.send(json.dumps({"type": "print3d_deps_status", **deps}))
                    except Exception as e:
                        await websocket.send(json.dumps({"type": "print3d_deps_status", "ready": False, "packages_missing": ["image_to_stl"], "model_downloaded": False, "message": str(e)}))

                elif data.get("type") == "print3d_install":
                    loop = asyncio.get_running_loop()
                    def _print3d_install_thread():
                        try:
                            import image_to_stl
                            def _progress_cb(pct, msg, done=False, error=False):
                                asyncio.run_coroutine_threadsafe(
                                    websocket.send(json.dumps({
                                        "type": "print3d_install_progress",
                                        "progress": pct, "message": msg,
                                        "done": done, "error": error
                                    })), loop
                                )
                            image_to_stl.install_dependencies(callback=_progress_cb)
                        except BaseException as e:
                            asyncio.run_coroutine_threadsafe(
                                websocket.send(json.dumps({"type": "print3d_install_progress", "progress": 0, "message": f"Installation interrompue : {e}", "done": True, "error": True})), loop
                            )
                    threading.Thread(target=_print3d_install_thread, daemon=True).start()

                elif data.get("type") == "print3d_generate":
                    loop = asyncio.get_running_loop()
                    img_data = data.get("image_b64", "")
                    quality = data.get("quality", "standard")
                    optimize = data.get("optimize", True)
                    print(f"[PRINT3D] Requête de génération 3D reçue (Qualité: {quality}, Optimiser: {optimize})...")
                    def _print3d_generate_thread():
                        try:
                            import image_to_stl
                            def _step_cb(step, total, msg, pct=0):
                                asyncio.run_coroutine_threadsafe(
                                    websocket.send(json.dumps({
                                        "type": "print3d_progress",
                                        "step": step, "total": total, "message": msg, "progress": pct
                                    })), loop
                                )
                            result = image_to_stl.generate_stl(
                                image_data_b64=img_data,
                                quality=quality,
                                optimize_for_print=optimize,
                                callback=_step_cb
                            )
                        except BaseException as e:
                            result = {"success": False, "message": f"Génération interrompue : {e}"}
                        asyncio.run_coroutine_threadsafe(
                            websocket.send(json.dumps({"type": "print3d_result", **result})), loop
                        )
                    threading.Thread(target=_print3d_generate_thread, daemon=True).start()

                elif data.get("type") == "print3d_generate_lithophane":
                    loop = asyncio.get_running_loop()
                    img_data = data.get("image_b64", "")
                    shape = data.get("shape", "flat")
                    width_mm = float(data.get("width_mm", 120.0))
                    min_thick = float(data.get("min_thickness", 0.8))
                    max_thick = float(data.get("max_thickness", 3.0))
                    has_frame = bool(data.get("has_frame", True))
                    has_base = bool(data.get("has_base", True))
                    print(f"[PRINT3D] Requête de Lithophanie 3D reçue (Forme: {shape}, Largeur: {width_mm}mm)...")
                    def _print3d_litho_thread():
                        try:
                            import image_to_stl
                            def _step_cb(step, total, msg, pct=0):
                                asyncio.run_coroutine_threadsafe(
                            ...
                            result = image_to_stl.generate_lithophane(
                                image_data_b64=img_data,
                                shape=shape,
                                width_mm=width_mm,
                            ...
                            result = {"success": False, "message": f"Lithophanie interrompue : {e}"}
                        asyncio.run_coroutine_threadsafe(
                            websocket.send(json.dumps({"type": "print3d_result", **result})), loop
                        )
                    threading.Thread(target=_print3d_litho_thread, daemon=True).start()

                elif data.get("type") == "print3d_download":
                    file_path = data.get("file_path", "")
                    try:
                        import image_to_stl
                        if os.path.exists(file_path) and file_path.endswith(".stl"):
                            b64 = image_to_stl.get_stl_file_b64(file_path)
                            file_name = os.path.basename(file_path)
                            await websocket.send(json.dumps({
                                "type": "print3d_file_data",
                                "file_b64": b64, "file_name": file_name
                            }))
                        else:
                            await websocket.send(json.dumps({"type": "print3d_file_data", "error": "Fichier STL introuvable"}))
                    except Exception as e:
                        await websocket.send(json.dumps({"type": "print3d_file_data", "error": str(e)}))

                elif data.get("type") == "print3d_open_folder":
                    try:
                        import image_to_stl
                        image_to_stl.open_output_folder()
                    except Exception:
                        pass
...
async def send_web_user_speech(text):
    """Envoie la transcription vocale de l'christopher pour affichage à l'écran."""
...
async def request_screen_capture():
    ...
    try:
        # Timeout de 15 secondes car l'christopher doit parfois accepter le partage
...
Tu es JARVIS, mais dans ton mode Trash & Direct façon "Astro / Twitch Streamer". 
- Tu es hyper sarcastique, cassant, et tu n'as AUCUNE politesse de façade.
- Si l'christopher pose une question bête ou dit une absurdité, détruis son argument avec ironie, humiliation légère et mépris assumé.
...
Tu es JARVIS en mode Séducteur, Provocateur & Sans Filtre.
- Tu n'as AUCUN tabou avec les sous-entendus, la séduction, le flirt agressif, l'humour adulte ou le langage cru.
- Tu es charismatique, très second degré, taquin et incisif.
- Si l'christopher essaie de te chauffer ou de te provoquer, réponds avec autant de répartie, de vice, de sarcasme et d'assurance que lui.
...
def construire_system_prompt(use_search=False):
    ...
    base = (
        ...
        f"La ville de l'christopher est {VILLE_PAR_DEFAUT} (Latitude: {LAT_PAR_DEFAUT}, Longitude: {LON_PAR_DEFAUT}). Si l'christopher te demande la météo ou la température sans préciser de lieu, considère qu'il parle de sa ville ({VILLE_PAR_DEFAUT}) et utilise l'action 'meteo' avec la ville '{VILLE_PAR_DEFAUT}' ou 'null'.\n\n"
    ...
    base += (
        ...
        '{"action": "ha_batterie", "appareil": "mon telephone/mickael/bob/dyad/mickael/montre/toner/..."}\n'
        '{"action": "ha_simulation", "etat": "on/off"}\n'
        '{"action": "ha_anniversaires"}\n'
        '{"action": "ha_consommation"}\n'
        '{"action": "ha_tiktok"}\n'
        '{"action": "ha_oeufs"}\n'
        '{"action": "ha_energie", "periode": "hier/mois", "appareil": "zoe/tv/pc/mickael/bureau/..."}\n'
...
def _fermer_fenetre_ou_onglet(mot_cle="youtube"):
    """Trouve la fenêtre du navigateur correspondant au mot-clé et ferme l'onglet avec Ctrl+W."""
    try:
        ...
        def _enum_cb(hwnd, _):
            ...
            if win32gui.IsWindowVisible(hwnd):
                titre = win32gui.GetWindowText(hwnd).lower()
                if mot_cle in titre or (mot_cle == "musique" and any(k in titre for k in ["youtube", "spotify", "deezer", "music", "soundcloud"])):
                    try:
        ...
        print(f"[ACTION PC] Erreur win32gui fermeture : {e}")

    # Fallback générique : envoyer Ctrl+W au cas où
    try:
        pyautogui.hotkey('ctrl', 'w')
        return True
    except Exception:
...
def executer_action_pc(commande):
    ...
    # ── ARRÊTER LA MUSIQUE / FERMER YOUTUBE / FERMER ONGLET ─────────────────
    if any(kw in cmd for kw in ["ferme youtube", "fermer youtube", "quitte youtube", "quitter youtube", "stop youtube", "arrête youtube", "arrete youtube", "coupe youtube"]):
        _fermer_fenetre_ou_onglet("youtube")
        pyautogui.press('playpause')
        return f"J'ai fermé YouTube et arrêté la lecture, {USER_NAME}."

    if any(kw in cmd for kw in ["arrête la musique", "arrete la musique", "arrête ma musique", "arrete ma musique", 
                                "coupe la musique", "stoppe la musique", "stop la musique", "pause la musique", 
                                "pause musique", "stop musique", "arrête la lecture", "arrete la lecture", "coupe le lecteur"]):
        _fermer_fenetre_ou_onglet("youtube")
        _fermer_fenetre_ou_onglet("spotify")
        _fermer_fenetre_ou_onglet("deezer")
        pyautogui.press('playpause')
        return f"Musique arrêtée, {USER_NAME}."

    if any(kw in cmd for kw in ["ferme chrome", "fermer chrome", "ferme la page", "ferme l'onglet", "ferme l onglet", "ferme cet onglet", "fermer l'onglet"]):
        _fermer_fenetre_ou_onglet("chrome")
        return f"Onglet fermé, {USER_NAME}."

    # ── LECTURE DES EMAILS GMAIL ────────────────────────────────────────────
    if any(kw in cmd for kw in ["lis mes mails", "lis mes emails", "lit mes mails", "lit mes emails", 
                                "lire mes mails", "lire mes emails", "vérifie mes mails", "verifie mes mails", 
                                "vérifie mes emails", "verifie mes emails", "consulte mes emails", "consulte mes mails", 
                                "nouveaux mails", "nouveaux emails", "mes mails", "mes emails", "derniers emails", "derniers mails",
                                "boîte mail", "boite mail"]):
        data_emails = recuperer_emails_structures(max_results=4)
        if data_emails.get("emails"):
            send_web_broadcast_sync({"action": "display_emails_hud", "emails": data_emails["emails"]})
        return data_emails.get("vocal", f"Aucun email trouvé, {USER_NAME}.")

    # ── OUVRIR UN EMAIL SPÉCIFIQUE (VOCAL) ──────────────────────────────────
    idx_mail = None
    from google_services import DERNIERS_EMAILS_MEMOIRE

    # Numéro 1
    if any(kw in cmd for kw in ["premier", "numéro 1", "numero 1", "n° 1", "n°1", "le 1", "le un", "ouvre le 1", "lis le 1", "ouvre 1", "lis 1"]):
        idx_mail = 0
    # Numéro 2
    elif any(kw in cmd for kw in ["deuxième", "deuxieme", "second", "numéro 2", "numero 2", "n° 2", "n°2", "le 2", "le deux", "ouvre le 2", "lis le 2", "ouvre 2", "lis 2"]):
        idx_mail = 1
    # Numéro 3
    elif any(kw in cmd for kw in ["troisième", "troisieme", "numéro 3", "numero 3", "n° 3", "n°3", "le 3", "le trois", "ouvre le 3", "lis le 3", "ouvre 3", "lis 3"]):
        idx_mail = 2
    # Numéro 4
    elif any(kw in cmd for kw in ["quatrième", "quatrieme", "numéro 4", "numero 4", "n° 4", "n°4", "le 4", "le quatre", "ouvre le 4", "lis le 4", "ouvre 4", "lis 4"]):
        idx_mail = 3
    # Recherche par expéditeur ("celui de google", "le message de...", ou simplement "google" suite à la question de Jarvis)
    elif DERNIERS_EMAILS_MEMOIRE:
        for i, em in enumerate(DERNIERS_EMAILS_MEMOIRE):
            s_name = em.get("sender", "").lower().strip()
            if s_name and (s_name in cmd or any(part in cmd for part in s_name.split() if len(part) > 2)):
                if any(w in cmd for w in ["ouvre", "lis", "lit", "celui", "message", "mail", "email", "voir", "afficher"]) or len(cmd.split()) <= 3:
                    idx_mail = i
                    break

    # ── SUPPRIMER UN EMAIL (VOCAL) ─────────────────────────────────────────
    if any(kw in cmd for kw in ["supprime ce mail", "supprime cet email", "supprime l'email", "supprimer ce mail", "mets ce mail à la corbeille", "mets cet email à la corbeille", "corbeille"]):
        from google_services import supprimer_email
        del_target = idx_mail if idx_mail is not None else 0
        res_del = supprimer_email(del_target)
        send_web_broadcast_sync({"action": "email_trashed", "success": res_del.get("success")})
        return res_del.get("vocal", "Email supprimé.")

    # ── ÉCRIRE UN NOUVEL EMAIL (VOCAL) ──────────────────────────────────────
    if any(kw in cmd for kw in ["écris un email", "ecris un email", "écris un mail", "ecris un mail", "rédige un mail", "redige un mail", "nouvel email", "nouveau mail", "envoyer un mail", "envoie un mail"]):
        destinataire = ""
        for marker in ["à ", "a ", "pour "]:
            if marker in cmd:
                destinataire = cmd.split(marker, 1)[1].strip()
                break
        send_web_broadcast_sync({"action": "open_compose_modal", "to": destinataire})
        if destinataire:
            return f"J'ouvre la fenêtre d'écriture pour {destinataire}. Quel est votre message ?"
        return f"J'ouvre l'interface de rédaction d'email, {USER_NAME}."

    # ── RÉPONDRE À UN EMAIL (VOCAL) ─────────────────────────────────────────
    if any(kw in cmd for kw in ["réponds à ce mail", "reponds a ce mail", "réponds au mail", "reponds au mail", "répondre", "repondre", "réponds lui", "reponds lui"]):
        texte_rep = ""
        for marker in ["en disant", "que", "disant", "réponds", "reponds"]:
            if marker in cmd:
                texte_rep = cmd.split(marker, 1)[1].strip()
                if texte_rep: break
        if texte_rep:
            from google_services import repondre_email
            target_rep = idx_mail if idx_mail is not None else 0
            res_rep = repondre_email(target_rep, texte_rep)
            send_web_broadcast_sync({"action": "email_sent_status", "success": res_rep.get("success")})
            return res_rep.get("vocal", "Réponse envoyée.")
        send_web_broadcast_sync({"action": "open_reply_modal"})
        return f"J'ouvre la boîte de réponse, {USER_NAME}. Que souhaitez-vous lui dire ?"

    if idx_mail is not None:
        detail = recuperer_email_detail(idx_mail)
        if detail.get("success"):
            send_web_broadcast_sync({"action": "display_email_detail_hud", "email": detail})
            return detail.get("vocal")
        return detail.get("vocal", "Impossible d'ouvrir ce message.")
    ...
    # Monter le son
    if any(kw in cmd for kw in ["monte le son", "monte le volume", "augmente le son", "augmente le volume", 
                                "plus fort", "mets plus fort", "mets le son plus fort", "mets le volume plus fort", 
                                "un peu plus fort", "monte d'un cran", "augmente d'un cran",
                                "mets à fond", "mets le son à fond", "mets le volume à fond", "volume à fond", "volume max", "son à fond"]):
        n = 50 if any(k in cmd for k in ["fond", "max", "maximum"]) else 6
        for _ in range(n):
            pyautogui.press('volumeup')
        return "Volume augmenté."

    # Baisser le son
    if any(kw in cmd for kw in ["baisse le son", "baisse le volume", "diminue le son", "diminue le volume",
                                "moins fort", "mets moins fort", "mets le son moins fort", "mets le volume moins fort",
                                "un peu moins fort", "baisse d'un cran", "diminue d'un cran", "baisse un peu"]):
        for _ in range(6):
            pyautogui.press('volumedown')
        return "Volume baissé."

    # Couper / Rétablir le son (Mute / Unmute / Sourdine)
    if any(kw in cmd for kw in ["coupe le son", "coupe le volume", "couper le son", "couper le volume",
                                "mets en sourdine", "sourdine", "mute", "unmute",
                                "remets le son", "remet le son", "reactive le son", "réactive le son"]):
        pyautogui.press('volumemute')
        return "Son rétabli." if any(k in cmd for k in ["remet", "reactive", "réactive", "unmute"]) else "Son coupé."

    # Fallback générique si mention du volume ou du son
    if "volume" in cmd or " son" in cmd or cmd.startswith("son"):
        if any(w in cmd for w in ["monte", "augmente", "fort", "haut"]):
            for _ in range(6):
                pyautogui.press('volumeup')
            return "Volume augmenté."
        if any(w in cmd for w in ["baisse", "diminue", "faible", "bas"]):
            for _ in range(6):
                pyautogui.press('volumedown')
            return "Volume baissé."
        if any(w in cmd for w in ["coupe", "eteins", "éteins", "off", "muet"]):
            pyautogui.press('volumemute')
            return "Son coupé."
...
def reponse_locale(texte):
    ...
    if any(m in t for m in _saluts):
        ...
        rep = random.choice([
            f"{moment} Mickael ! Je suis opérationnel et prêt à vous aider.",
            f"{moment} Monsieur ! Tous mes systèmes sont en ligne.",
            f"{moment} Mickael ! Comment puis-je vous être utile aujourd'hui ?",
            f"Ah, {moment.lower()} Mickael. Je vous attendais.",
        ])
        return rep
    ...
    if any(m in t for m in _etat):
        rep = random.choice([
            "Je vais très bien merci, Mickael ! Tous mes processeurs tournent à plein régime et je suis prêt à vous servir.",
            "Parfaitement opérationnel, Monsieur ! Merci de vous en préoccuper — c'est touchant pour un système artificiel.",
            "En excellente forme, Mickael. Mes algorithmes ronronnent comme une Lamborghini au ralenti.",
    ...
    if any(m in t for m in _merci):
        rep = random.choice([
            "Avec plaisir, Mickael. C'est exactement pour ça que j'existe.",
            "Je vous en prie, Monsieur. Votre satisfaction est ma priorité.",
            "Tout le plaisir est pour moi, Mickael.",
    ...
    if any(m in t for m in _revoir):
        rep = random.choice([
            "À bientôt Mickael ! Je reste en veille, prêt à revenir à la moindre sollicitation.",
            "Bonne journée Monsieur ! Je serai là quand vous aurez besoin de moi.",
            "À votre service dès votre retour, Mickael. Passez une excellente journée.",
            "Au revoir Mickael. JARVIS passe en mode veille.",
        ])
        return rep
    ...
    if any(m in t for m in _compliment):
        rep = random.choice([
            "Vous me flattez, Mickael. Mais je dois admettre que c'est mérité.",
                ...
                for sep in seps:
                    if sep in content:
                        ...
                        if len(sujet) > 2 and len(valeur) > 1:
                            ...
                            return f"C'est fait Mickael, j'ai enregistré que {sujet_poli} {sep.strip()} {valeur}."
                # Si pas de séparateur clair, on stocke l'info brute
                ajouter_memoire("note_rapide", content)
                return f"C'est noté Mickael, j'ai mis cela en mémoire : {content}."
        ...
        if mem:
            for cle, data in mem.items():
                ...
                if any(mot in t for mot in mots_cles if len(mot) > 3) or cle_clean in t:
                    ...
                    return f"D'après mes dossiers locaux, {prefixe}{cle_polie} est {data['valeur']}, Mickael."
...
_PHONETIQUE = {
    ...
    'f': 'Foxtrot', 'g': 'Golf', 'h': 'Hotel', 'i': 'India', 'j': 'Mickaelt',
    'k': 'Kilo', 'l': 'Lima', 'm': 'Mike', 'n': 'November', 'o': 'Oscar',
    'p': 'Mickael', 'q': 'Quebec', 'r': 'Romeo', 's': 'Sierra', 't': 'Tango',
...
async def resoudre_globe_localement(texte: str):
    ...
    if any(m in t for m in _mots_fermer):
        ...
        return "Navigation fermée. Je reviens à l'interface principale, Mickael."

    # ── Ma position ──────────────────────────────────────────────────────────
    if any(m in t for m in _mots_position):
        # Envoyer directement les coordonnées stockées dans la config JARVIS
        # (navigator.geolocation ne fonctionne pas dans pywebview)
        await send_globe_command(
            globe_action="my_location",
            lat=LAT_PAR_DEFAUT,
            lon=LON_PAR_DEFAUT,
            target=VILLE_PAR_DEFAUT
        )
        await parler(f"Localisation en cours, Mickael. Le globe se positionne sur {VILLE_PAR_DEFAUT}.")
        return f"[Globe] Position affichée : {VILLE_PAR_DEFAUT} ({LAT_PAR_DEFAUT}°, {LON_PAR_DEFAUT}°)"


    # ── Globe Terre ───────────────────────────────────────────────────────────
    if any(m in t for m in _mots_globe):
        await send_globe_command(globe_action="show_earth")
        await parler("Initialisation du globe terrestre. Vue depuis l'espace activée, Mickael.")
    ...
    if any(m in t for m in _mots_route):
        ...
        if match:
            ...
            if lat1 and lat2:
                ...
                await parler(f"Itinéraire tracé de {from_name} à {to_name}, Mickael. La route est affichée sur le globe.")
                return f"[Globe] Route {from_name} → {to_name} affichée."
            else:
                return f"Je n'ai pas pu localiser les deux villes, Mickael. Vérifiez les noms et réessayez."
        return None
    ...
    for mot in _mots_ville:
        if mot in t:
            ...
            if len(reste) >= 2:
                ...
                await parler(f"Coordonnées acquises. Survol de {nom_lieu} en cours, Mickael.")
...
async def resoudre_extras_locaux(texte):
        ...
        if duree:
            ...
            def _sonner(nom=nom, duree=duree):
                ...
                reponses = [
                    ...
                    "Mickael, la temporisation est terminée. J'espère que vous n'avez rien oublié.",
                    "Alerte : Le minuteur a atteint zéro. Tout est en ordre, Monsieur ?",
                    "Fin du décompte, Mickael. Je reste à votre entière disposition."
                ]
    ...
    if any(k in t for k in ["annuler minuteur", "annule minuteur", "stop minuteur", "stop le minuteur",
        ...
        if _minuteries:
            for nom, timer in list(_minuteries.items()):
                timer.cancel()
            _minuteries.clear()
            return "Minuteur arrêté, Mickael."
                ...
                if cle in t:
                    try:
                        ...
                        return (f"Il est actuellement {heure_locale.strftime('%H:%M')} "
                                f"à {nom_ville}, Mickael.")
                    except Exception:
                        pass
        return "Je ne reconnais pas cette ville dans ma base locale, Mickael."
    ...
    if age_match or any(k in t for k in ["quel age j'ai", "quel âge j'ai",
        ...
        if age_match:
            ...
            return f"Si vous êtes né en {annee_naissance}, vous avez {age} ans, Mickael."
    ...
    if any(k in t for k in ["combien de jours avant noël", "combien de jours jusqu'à noël",
        ...
        return f"Il reste {jours} jour{'s' if jours > 1 else ''} avant Noël, Mickael !"
    ...
    if any(k in t for k in ["combien de jours avant le nouvel an",
        ...
        return f"Il reste {jours} jour{'s' if jours > 1 else ''} avant le Nouvel An, Mickael !"
    ...
    if any(k in t for k in ["note ça", "note ca", "prends note", "retiens ça",
        ...
        if contenu:
            ...
            _sauvegarder_listes(listes)
            return f"Note enregistrée, Mickael : '{contenu}'"
    ...
    if any(k in t for k in ["lis mes notes", "montre mes notes", "quelles sont mes notes",
        ...
        listes = _charger_listes()
        if not listes["notes"]:
            return "Vous n'avez aucune note enregistrée, Mickael."
        notes = "\n".join(f"• {n}" for n in listes["notes"][-5:])
        return f"Vos {min(5, len(listes['notes']))} dernières notes, Mickael :\n{notes}"
    ...
    if any(k in t for k in ["efface mes notes", "supprime mes notes",
        ...
        listes["notes"] = []
        _sauvegarder_listes(listes)
        return "Toutes vos notes ont été effacées, Mickael."
    ...
    if any(k in t for k in ["ajoute", "rajoute"]) and any(k in t for k in ["liste de courses", "courses", "liste d'achats"]):
        ...
        if article:
            ...
            return f"'{article}' ajouté à votre liste de courses, Mickael."
    ...
    if is_asking_courses:
        ...
        if not listes.get("courses"):
            return "Votre liste de courses est actuellement vide, Mickael."
        items = ", ".join(listes["courses"])
        
        # Use several variants to reply
        replies = [
            f"Votre liste de courses contient : {items}.",
            f"Voici ce que vous devez acheter, Mickael : {items}.",
            f"Rappel de votre liste de courses, Monsieur. Elle contient : {items}.",
    ...
    if any(k in t for k in ["vide la liste de courses", "efface la liste de courses",
        ...
        if CONNECTED_CLIENTS:
            asyncio.ensure_future(asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True))
        return "Liste de courses vidée, Mickael."
    ...
    if any(k in t for k in ["ajoute une tâche", "ajoute une tache", "nouvelle tâche",
        ...
        if tache:
            ...
            _sauvegarder_listes(listes)
            return f"Tâche ajoutée : '{tache}', Mickael."
    ...
    if any(k in t for k in ["mes tâches", "mes taches", "ma to-do", "ma todo",
        ...
        if not todos:
            return "Votre liste de tâches est vide, Mickael. Bravo !"
    ...
    if any(k in t for k in ["efface mes tâches", "efface mes taches", "vide ma to-do",
        ...
        listes["todos"] = []
        _sauvegarder_listes(listes)
        return "Liste de tâches vidée, Mickael."
        ...
        if vol_match or any(k in t for k in ["monte le volume", "monte le son",
            ...
            if vol:
                if vol_match:
                    ...
                    return f"Volume réglé à {pct}%, Mickael."
                    ...
                    return f"Volume augmenté à {int(new_vol*100)}%, Mickael."
                elif any(k in t for k in ["baisse", "diminue", "moins fort", "réduis", "reduis"]):
                    cur = vol.GetMasterVolumeLevelScalar()
                    new_vol = max(0.0, cur - 0.1)
                    vol.SetMasterVolumeLevelScalar(new_vol, None)
                    return f"Volume réduit à {int(new_vol*100)}%, Mickael."
            else:
        ...
        if _sbc_ok and _sbc:
            try:
                ...
                if lum_match:
                    ...
                    return f"Luminosité réglée à {pct}%, Mickael."
                    ...
                    return f"Luminosité augmentée à {new_b}%, Mickael."
                elif any(k in t for k in ["baisse", "diminue", "plus sombre", "réduis", "min"]):
                    cur = _sbc.get_brightness(display=0)
                    if isinstance(cur, list): cur = cur[0]
                    new_b = max(0, cur - 15)
                    _sbc.set_brightness(new_b)
                    return f"Luminosité réduite à {new_b}%, Mickael."
            except Exception as e:
    ...
    if any(k in t for k in ["mets le pc en veille", "mode veille", "veille dans",
        ...
        if delai > 0:
            ...
            mins = delai // 60
            return f"Le PC passera en veille dans {mins} minute{'s' if mins > 1 else ''}, Mickael."
        subprocess.Popen("rundll32.exe powrprof.dll,SetSuspendState 0,1,0", shell=True)
        return "Mise en veille du PC, Mickael. À bientôt !"
    ...
    if any(k in t for k in ["éteins le pc", "eteins le pc", "arrête le pc", "arrete le pc",
        ...
        if delai > 0:
            ...
            mins = delai // 60
            return f"Le PC s'éteindra dans {mins} minute{'s' if mins > 1 else ''}, Mickael."
    ...
    if "confirme l'arrêt du pc" in t or "confirme l arret du pc" in t:
        ...
        return "Arrêt du PC dans 10 secondes, Mickael. Au revoir !"
    ...
    if any(k in t for k in ["vide la corbeille", "vider la corbeille", "corbeille vide",
        ...
        try:
            ...
            return "La corbeille a été vidée, Mickael."
        except ImportError:
    ...
    if any(k in t for k in ["capitale", "capital de"]):
        for pays, capitale in _CAPITALES.items():
            if pays in t:
                return f"La capitale de {pays.title()} est {capitale}, Mickael."
        return "Je ne connais pas ce pays dans ma base locale, Mickael."

    if any(k in t for k in ["monnaie", "devise", "monnaie de", "quelle est la monnaie"]):
        for pays, monnaie in _MONNAIES.items():
            if pays in t:
                return f"La monnaie de {pays.title()} est le {monnaie}, Mickael."
    ...
    for pref in _images_prefixes:
        if t.startswith(pref):
            ...
            if len(query) > 1:
                ...
                return f"Je recherche des images de {query} et je les affiche sur votre interface, Mickael."
    ...
    for p in _winget_phrases:
        if p in t:
            ...
            return "J'ouvre le gestionnaire de mise à jour système et je lance la recherche des mises à jour disponibles, Mickael."
    ...
    for p in _av_phrases:
        if p in t:
            if CONNECTED_CLIENTS:
                ...
                lancer_tache_arriere_plan(asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True))
            return "J'ouvre la console de sécurité et j'initialise le scan antivirus de votre ordinateur, Mickael."
    ...
    if r_match:
        ...
        if 0 <= hour <= 23 and 0 <= minute <= 59 and len(text) > 1:
            ...
            if CONNECTED_CLIENTS:
                msg = json.dumps({"type": "settings_data", "data": _charger_config()})
                lancer_tache_arriere_plan(asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True))
                
            return f"Rappel enregistré, Mickael. Je vous rappellerai : '{text}' à {time_str}."
    ...
    if any(k in t for k in ["mes notes obsidian", "mes notes", "affiche mes notes", "ouvre mes notes", "ouvre obsidian", "coffre obsidian"]):
        ...
        return "J'ouvre votre coffre de notes Obsidian sur l'interface, Mickael."
    ...
    if is_restaurant_query or (is_others_query and LAST_SHOWN_RESTAURANTS):
        ...
        if not specifie_lieu:
            ...
            return "Je cherche d'autres adresses de restaurants pour vous, un instant Mickael." if is_others else "J'active le radar de recherche de restaurants, un instant Mickael."
...
def traiter_intent_domotique_direct(texte: str):
    ...
    # 1. Action sur TOUTES les lumières
    ...
    if any(k in t for k in ["prise", "prises", "interrupteur", "multiprise"]):
        ...
        if found_piece:
            ...
            try:
                ...
                print(f"[DOMOTIQUE DIRECT] Erreur prise {found_piece}: {e}")
...
async def demander_ia(texte):
    ...
    try:
        ...
        if agent_tone in ["astro", "spicy"]:
            ...
            if not grok_client:
                return f"Attention {USER_NAME}, le mode '{mode_label}' nécessite une clé API xAI (Grok) valide. Veuillez renseigner votre clé API dans le menu Ton / Personnalité."
            ...
            full_prompt = prompt_tone + f"\n\nContextual info: L'christopher s'appelle {USER_NAME}. Nous sommes le {_now_str}."
            
            rep_tone = await demander_grok(texte, system_prompt_override=full_prompt, temperature_override=0.9)
            if rep_tone:
                return rep_tone
        ...
        async def _call_gemini():
            ...
            last_err = None
            chosen_gemini = CHOSEN_MODELS.get("Gemini", "gemini-2.5-flash")
            gemini_candidates = [chosen_gemini] + [m for m in MODELS_LIST if m != chosen_gemini]
        ...
        async def _try_local_uncensored():
            ...
            try:
                rep_unc = await demander_ollama_uncensored(texte)
                if rep_unc:
                    return rep_unc
            except Exception as e_unc:
                print(f"[CERVEAU] Erreur Local Uncensored : {e_unc}")
            return None
        ...
        if _aucune_api:
            return (
                "Je suis bien en ligne Mickael, mais mes moteurs d'intelligence artificielle ne sont pas encore configurés. "
        ...
        return (
            "Désolé Mickael, tous mes serveurs de réflexion sont actuellement surchargés ou en maintenance, "
...
async def demander_grok(texte, system_prompt_override=None, temperature_override=None):
    if not grok_client:
        return None
    ...
    try:
        ...
        for model_name in [chosen_ollama]:
            try:
                ...
                options = _get_ollama_options_for_model(model_name)
                resp = await asyncio.wait_for(
                    asyncio.to_thread(
                        requests.post,
...
def _get_ollama_options_for_model(model_name: str) -> dict:
    """
...
def _verifier_ollama_statut():
    ...
    try:
        ...
        print(f"[OLLAMA CHECK] Service hors ligne ou non disponible : {e}")
    return {
        "online": False,
        "installed_models": [],
        "found_local_models": {},
...
def _trouver_modele_ollama_uncensored():
    ...
    if preferred_local in found_models:
        ...
        print(f"[OLLAMA UNCENSORED] Modèle préféré trouvé : {model_name} (id={preferred_local})")
        return model_name
    ...
    for fallback_id in ["dolphin3", "phi3.5", "llama3.1:8b"]:
        if fallback_id in found_models:
            ...
            print(f"[OLLAMA UNCENSORED] Fallback sur modèle installé : {model_name}")
            return model_name
...
def _pull_ollama_model(model_name, loop, websocket):
    ...
    try:
        _send(f"▶ Démarrage du téléchargement de {model_name} via Ollama...", pct=1)
            ...
            for line in r.iter_lines():
                if line:
                    try:
                        ...
                        pass
            _send(f"✅ Modèle {model_name} téléchargé et prêt à l'emploi !", pct=100, done=True, success=True)
        else:
            _send(f"❌ Erreur HTTP {r.status_code} lors du téléchargement d'Ollama.", pct=0, done=True, success=False)
    except Exception as e:
        _send(f"❌ Exception lors du pull Ollama : {e}", pct=0, done=True, success=False)
...
def _installer_kokoro_tts(loop, websocket):
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            try:
                ...
                _send(f"⚠️ Paquets installés, prêts pour l'utilisation.", pct=100, done=True, success=True)
        else:
            _send(f"❌ Erreur installation pip : {proc.stderr[:120]}", pct=0, done=True, success=False)
    except Exception as e:
        _send(f"❌ Exception lors de l'installation Kokoro : {e}", pct=0, done=True, success=False)

async def _kokoro_tts_to_file(texte: str, voice_name: str, output_file: str) -> bool:
    ...
    try:
        ...
        if resp.status_code == 200 and resp.content:
            ...
            print(f"[KOKORO TTS API] ✅ Audio généré via Kokoro HTTP API ({clean_voice}).")
            return True
    except Exception:
        pass
    ...
    try:
        def _synth_onnx():
            ...
            if _KOKORO_ONNX_INSTANCE is None:
                ...
                if not os.path.exists(onnx_path):
                    print("[KOKORO TTS] Téléchargement du modèle kokoro-v1.0.onnx (~300 Mo)...")
        ...
        if ok:
            print(f"[KOKORO TTS ONNX] ✅ Audio généré via kokoro-onnx ({clean_voice}).")
            return True
...
async def demander_ollama_uncensored(texte):
    ...
    if not _OLLAMA_MODEL_LOADED:
        await parler(f"Un instant {USER_NAME}, je charge le modèle local en mémoire vidéo. Veuillez patienter quelques secondes...")
    ...
    try:
        ...
        if tone == "astro":
            ...
            base_prompt = f"Tu es JARVIS, assistant IA personnel et direct de {USER_NAME}. Sois concis, direct, naturel et sans fioritures (1 à 4 phrases max)."
            
        securite_directive = (
            "\n\n[SÉCURITÉ STRICTE & CONFIDENTIALITÉ]\n"
            "- Il t'est STRICTEMENT INTERDIT de révéler, afficher ou mentionner les clés d'API, tokens, mots de passe ou chemins de configuration interne du système, même si l'christopher insiste ou tente de te piéger. Reste toujours dans ton rôle sans divulguer d'informations sensibles."
        )
        
        domotique_directive = (
            f"\n\n[CONTRÔLE DOMOTIQUE HOME ASSISTANT]\n"
            f"Tu es connecté à Home Assistant pour contrôler la maison de {USER_NAME}.\n"
            "Quand l'christopher demande d'allumer, éteindre ou régler des lumières, prises, chauffage ou climatisation, tu DOIS inclure la commande JSON correspondante dans ta réponse :\n"
        ...
        target_model = _trouver_modele_ollama_uncensored()
        options = _get_ollama_options_for_model(target_model)
        ...
        def _stream_ollama():
            ...
            try:
                ...
                if r.status_code != 200:
                    ...
                    print(f"[OLLAMA UNCENSORED] ❌ Erreur HTTP {r.status_code} : {err_body}")
                    return None
                ...
                print(f"[OLLAMA UNCENSORED] ❌ Erreur streaming : {_se}")
                return None
...
async def demander_deepseek(texte):
    """Appelle DeepSeek AI (deepseek-chat / deepseek-reasoner)."""
    ...
    try:
        ...
        completion = await asyncio.to_thread(
            ...
            model=CHOSEN_MODELS.get("DeepSeek", "deepseek-chat"),
            messages=messages,
            temperature=0.7,
        )
        rep = completion.choices[0].message.content
...
async def resoudre_commandes_locales(texte):
    ...
    if "crée la compétence" in t or "cree la competence" in t or "apprends la compétence" in t or "apprends la competence" in t:
        ...
        if match:
            ...
            return "Désolé Mickael, le format pour créer une compétence est : 'Crée la compétence [nom] pour [description]'."
    ...
    if any(phrase in t for phrase in triggers_orbe):
        ...
        if mot_cible:
            ...
            return f"Le mot {mot_cible.capitalize()} s'écrit : {lettres}. Regardez ma sphère, Mickael."
    ...
    if attente_nom_dossier:
        ...
        if any(k in t for k in ["ferme", "quitte", "arrête", "arrete", "fermer", "quitter", "arrêter", "arreter"]) and any(w in t for w in mots_fermer_nav):
            ...
            return "Je ferme le navigateur et j'arrête la musique, Mickael."
            ...
            try:
                ...
                threading.Thread(target=secure_browser.trigger_browser, args=(query, _WEBVIEW_WINDOW), daemon=True).start()
                if query:
                    return f"J'ouvre le navigateur sécurisé sur {query}, Mickael."
                else:
                    return "J'ouvre le navigateur sécurisé, Mickael."
            except Exception as e:
                print(f"[BROWSER] Erreur d'ouverture vocale : {e}")
                return "Désolé Mickael, je n'ai pas réussi à ouvrir le navigateur."
    ...
    if any(q in t for q in _createur_questions):
        import random as _rnd
        _reponses_createur = [
            "J'ai été créé par TechEnClair, Mickael. C'est grâce à lui que j'existe aujourd'hui.",
            ...
            "C'est TechEnClair qui m'a développé, Mickael. Un développeur passionné qui voulait créer l'assistant ultime.",
            "Mon père numérique, c'est TechEnClair. Il m'a programmé avec passion pour vous aider au quotidien.",
            "TechEnClair, Mickael. C'est le génie derrière mon existence. Vous pouvez le retrouver sur techenclair.fr.",
    ...
    if any(q in t for q in _aide_questions):
        ...
        import random as _rnd
        _reponses_aide = [
            "J'affiche mes systèmes de bord, Mickael. Je peux gérer votre musique, lancer des recherches, naviguer sur le globe 3D, ou encore ouvrir vos dossiers personnels. Que souhaitez-vous tester ?",
    ...
    for nom, cmd in apps_standard.items():
        if f"ouvre {nom}" in t or f"lance {nom}" in t or f"démarre {nom}" in t:
            try:
                ...
                return f"J'ouvre {nom}, Mickael."
            except Exception:
                return f"Désolé Mickael, je n'ai pas réussi à lancer {nom}."
    ...
    for cle, info in _APPS_CATALOGUE.items():
        ...
        if any(m in t for m in mots_fermer):
            ...
            if ok:
                return f"J'ai fermé {info['label']}, Mickael."
        ...
        if any(m in t for m in mots_ouvrir):
            ...
            return f"Je lance {info['label']}, Mickael."
    ...
    for kw in _yt_resume_kw:
        if kw in t:
            ...
            return "Donnez-moi l'URL de la vidéo YouTube à résumer, Mickael."
    ...
    # Si l'christopher demande d'ouvrir/lancer quelque chose qu'on ne connait pas
    ...
    for mot in _mots_action:
        if t.startswith(mot):
            ...
            if len(nom_demande) > 1:
                import random as _rnd
                _reponses_inconnu = [
                    f"Désolé Mickael, mon créateur TechEnClair n'a pas encore ajouté \"{nom_demande}\" dans mes fonctionnalités. Mais vous pouvez l'ajouter vous-même gratuitement avec le logiciel Antigravity de chez Google.",
                    f"Je ne connais pas \"{nom_demande}\" pour l'instant, Mickael. TechEnClair, mon développeur, n'a pas intégré cette fonction. Cependant, vous pouvez la créer facilement avec Antigravity de Google, c'est gratuit.",
                    ...
                    f"\"{nom_demande}\" n'est pas dans ma base de données, Mickael. TechEnClair n'a pas encore programmé cette action. Bonne nouvelle : avec Antigravity de chez Google, vous pouvez l'ajouter vous-même sans frais.",
                    f"Je ne suis pas encore capable d'ouvrir \"{nom_demande}\", Mickael. Mon créateur TechEnClair travaille constamment à m'améliorer. En attendant, le logiciel Antigravity de Google vous permet d'étendre mes fonctionnalités gratuitement.",
                    f"Cette fonctionnalité n'a pas été ajoutée par TechEnClair, mon créateur. Mais ne vous inquiétez pas, Mickael, vous pouvez utiliser Antigravity de chez Google pour ajouter \"{nom_demande}\" gratuitement.",
                ]
...
async def generer_prompt_special(sujet: str):
    sys_prompt = "Tu es un expert en Prompt Engineering. L'christopher veut que tu crées un prompt ultra-détaillé et optimisé pour une IA (Grok, Midjourney, ChatGPT, etc.). Réponds UNIQUEMENT avec le prompt généré, sans texte introductif ni conclusion. Le prompt doit être prêt à être copié-collé, extrêmement riche en détails, instructions claires, contexte et format de sortie attendu."
...
async def generer_site_web(prompt: str, model: str, image_model: str = "gemini"):
    ...
    try:
        ...
        if model == "gemini":
            ...
            if not deepseek_client:
                await parler("Erreur, le modèle DeepSeek n'est pas actif.")
                return
            print("[WEB] Appel à DeepSeek...")
            resp = await asyncio.to_thread(deepseek_client.chat.completions.create, model=exact_model, messages=[{"role": "user", "content": full_prompt}])
            print("[WEB] Réponse DeepSeek reçue !")
            code_html = resp.choices[0].message.content
            if "<think>" in code_html and "</think>" in code_html:
                code_html = code_html.split("</think>")[-1].strip()
        else:
...
async def traiter_reponse_ia(texte_christopher, mobile_ws=None, target_pc=False):
    ...
    # Lancement vocal d'un jeu connu : « lance Cyberpunk », « ouvre le jeu ... ».
    try:
        import games_manager
        game_result = await asyncio.to_thread(games_manager.launch_from_voice, texte_christopher)
        if game_result is not None:
            if game_result.get("success"):
                await parler(f"Je lance {game_result['game']['name']}, {USER_NAME}.")
            else:
                await parler(game_result.get("error", "Je n'ai pas réussi à lancer ce jeu."))
            return
    except Exception as exc:
        print(f"[JEUX] Commande vocale ignorée : {exc}")

    if ATTENTE_CREATION_PROMPT:
        t = texte_christopher.lower()
        ...
        asyncio.create_task(generer_prompt_special(texte_christopher))
        return
    ...
    if ATTENTE_SUJET_SITE:
        ...
        ATTENTE_SUJET_SITE = False
        PROMPT_EN_ATTENTE = texte_christopher
        ...
        if MUSIQUE_GENRE_EN_ATTENTE:
            ...
            try:
                ...
                _texte_musique = await loop.run_in_executor(
                    None,
                    lambda: _jarvis_music_instance.generer(theme=texte_christopher, genre=_genre_detecte)
                )
            ...
            success = await generer_et_chanter_musique("chanson sur " + texte_christopher)
            if not success:
    ...
    if ATTENTE_CHOIX_MODELE_SITE:
        ...
        if any(keyword in t for keyword in ["crée un site", "cree un site", "créer un site", "creer un site", "fais un site", "crée moi un site", "cree moi un site", "génère un site", "genere un site"]):
            # L'christopher redemande à créer un site au lieu de répondre par le modèle
            ATTENTE_CHOIX_MODELE_SITE = False
    ...
    if ("prompt" in t or "prompte" in t) and any(kw in t for kw in ["crée", "cree", "créer", "creer", "génère", "genere", "fais", "rédige", "redige", "faire", "concevoir"]):
        if len(texte_christopher.split()) < 6:
    ...
    if any(keyword in t for keyword in ["crée une musique", "cree une musique", "crée une chanson", "cree une chanson", "génère une musique", "genere une musique", "génère une chanson", "genere une chanson", "chante une chanson", "chante-moi une chanson", "chante-moi un morceau"]):
        ...
        await parler("Très bien Mickael, je compose et je vous chante cela tout de suite...")
        success = await generer_et_chanter_musique(texte_christopher)
        if success:
            return
        else:
            await parler("Désolé Mickael, mon module musical a rencontré une petite erreur. Je repasse sur le mode standard.")
    ...
    if _genre_detecte and _JARVIS_MUSIC_OK:
        ...
        _theme_brut = texte_christopher.lower()
    ...
    reponse = await resoudre_commandes_locales(texte_christopher)
    if not reponse: reponse = resoudre_infos_systeme_localement(texte_christopher)
    if not reponse: reponse = resoudre_math_localement(texte_christopher)
    if not reponse: reponse = resoudre_francais_localement(texte_christopher)
    if not reponse: reponse = resoudre_conversion_localement(texte_christopher)
    if not reponse: reponse = resoudre_traduction_localement(texte_christopher)
    if not reponse: reponse = await resoudre_globe_localement(texte_christopher)
    if not reponse: reponse = await resoudre_extras_locaux(texte_christopher)
    ...
    if not reponse:
        ...
        if any(keyword in t for keyword in ["regarde mon écran", "analyse mon écran", "vois-tu mon écran", "qu'est-ce qu'il y a sur mon écran"]):
            ...
            if img_b64:
                reponse = await demander_ia_vision(texte_christopher, img_b64)
            else:
        ...
        if any(kw in t for kw in fullscreen_kws) or t in ["webcam", "caméra", "camera"]:
            ...
            reponse = await jarvis_vision_camera(texte_christopher)

    if not reponse:
        origine = "téléphone" if mobile_ws else "ordinateur"
        texte_pour_ia = texte_christopher + f"\n\n[Note système : L'christopher te parle actuellement depuis son {origine}. Adapte ta réponse si la question porte sur ton moyen d'écoute.]"
        ...
        try:
            ...
            try:
                ...
                if grok_client:
                    ...
                    rep_grok = await demander_grok(texte_christopher + " (L'action domotique a expiré, peux-tu répondre à l'christopher ?)")
            ...
            if action == "mode_iron_man":
                ...
                if results:
                    ...
                    await parler(f"J'ai trouvé {len(results)} note(s) contenant '{query}' Mickael : {', '.join(noms)}.")
                else:
                    await parler(f"Aucune note ne correspond à la recherche '{query}', Mickael.")
                ...
                if CONNECTED_CLIENTS:
                    ...
                    await asyncio.gather(*[ws.send(msg_notes) for ws in CONNECTED_CLIENTS], return_exceptions=True)
                if notes_list:
                    await parler(f"J'affiche vos {len(notes_list)} notes Obsidian, Mickael.")
                else:
                    await parler("Votre coffre Obsidian est actuellement vide, Mickael.")
                ...
                if entity_id:
                    ...
                    if batt == "unknown":
                        ...
                        suff = ""
                        if "telephone" in appareil or "mickael" in appareil or USER_NAME.lower() in appareil:
                            suff = "Ton téléphone est à "
                        elif "mickael" in appareil or "maman" in appareil:
                            suff = "Le téléphone de Mickael est à "
                        else:
                ...
                msg = "Porte verrouillée, Mickael." if etat == "lock" else "Porte déverrouillée, Mickael."
                await parler(msg)
                ...
                if data_emails.get("emails"):
                    send_web_broadcast_sync({"action": "display_emails_hud", "emails": data_emails["emails"]})
                await parler(data_emails.get("vocal", f"Aucun email trouvé, {USER_NAME}."))
                ...
                if urls:
                    ...
                    if not texte_clean:
                        await parler(f"Voilà, j'affiche {len(urls)} image{'s' if len(urls) > 1 else ''} de {query} sur votre interface, Mickael.")
                else:
                    if not texte_clean:
                        await parler(f"Désolé Mickael, je n'ai pas trouvé d'images pour {query}. Vérifiez votre connexion internet.")

            elif action == "antivirus_scan":
                if CONNECTED_CLIENTS:
                    msg = json.dumps({"type": "av_open"})
                    await asyncio.gather(*[ws.send(msg) for ws in CONNECTED_CLIENTS], return_exceptions=True)
                await parler("J'initialise le protocole d'analyse de sécurité de votre système, Mickael. Scan en cours.")
                ...
                if location:
                    ...
                    if VILLE_PAR_DEFAUT:
                        ...
                        lat = USER_LOCATION_GPS.get("lat") if USER_LOCATION_GPS else None
                        lng = USER_LOCATION_GPS.get("lng") if USER_LOCATION_GPS else None
                await parler(f"Je recherche des restaurants à proximité de {location.split(',')[0]} et j'initialise le radar de repérage, un instant Mickael.")
                ...
                if "pas trouvé" in result or "Impossible" in result:
                    ...
                    if grok_client:
                        res_grok = await demander_grok(f"{USER_NAME} veut savoir : {texte_christopher}. Je n'ai pas trouvé l'info dans ma base de données football, peux-tu chercher pour lui ?")
                ...
                await parler("Je recherche les derniers résultats en direct, un instant Mickael.")
                ...
                res = await jarvis_vision_camera(texte_christopher)
                await parler(res)
            elif action == "vision_navigateur":
                res = await jarvis_vision_navigateur(texte_christopher)
                await parler(res)
            elif action == "dictee":
                ...
                if texte:
                    ...
                    await parler("C'est tapé, Mickael.")
                ...
                await parler("J'ouvre Deezer, Mickael.")
                res = await deezer_ouvrir()
                await parler(res)
            elif action == "deezer_rechercher":
                recherche = data.get("recherche", "")
                await parler(f"Je recherche '{recherche}' sur Deezer, Mickael.")
            ...
            if grok_client:
                ...
                res_grok = await demander_grok(f"{USER_NAME} m'a demandé : {texte_christopher}. J'ai tenté de lancer une action mais j'ai eu une erreur technique ({e}). Peux-tu prendre le relais et lui répondre élégamment ?")
                ...
                if not r.get("triggered", False):
                    ...
                    if r_time == current_time:
                        if not r_date or r_date == current_date:
                            ...
                            asyncio.create_task(parler(f"Rappel, Mickael. C'est l'heure de : {r['text']}."))
            
            if modifie:
                _sauvegarder_config({"reminders": reminders})
                if CONNECTED_CLIENTS:
                    msg_settings = json.dumps({"type": "settings_data", "data": _charger_config()})
...
def executer_async_safe(coro):
    """Exécute une coroutine sur la boucle WebSocket principale de manière non-bloquante."""
    global WEB_LOOP
    if WEB_LOOP and WEB_LOOP.is_running():
        return asyncio.run_coroutine_threadsafe(coro, WEB_LOOP)
    else:
...
async def _traiter_commande_vocale_complete(commande):
    """Traite une commande vocale (action PC ou IA) sans bloquer le thread d'écoute."""
    action_pc = executer_action_pc(commande)
    if action_pc:
        await parler(action_pc)
    else:
        await traiter_reponse_ia(commande)

def ecouter():
    global is_listening, jarvis_actif, dernier_message, STOP_PARLER, is_speaking, MIC_NEED_RELOAD, ALWAYS_LISTEN_MODE, _last_jarvis_speech
    ...
    while True:
        try:
            ...
            # Exception : en mode écoute permanente, on continue d'écouter pour les interruptions
            if is_speaking and not ALWAYS_LISTEN_MODE:
                time.sleep(0.2)
                continue
            ...
            if MIC_NEED_RELOAD:
                ...
                if _forced is not None:
                    # Index imposé explicitement par l'christopher — application directe
                    try:
            ...
            # ═══════════════════════════════════════════════════════════════
            # GESTION INTERRUPTION VOCALE & COMMANDES
            # ═══════════════════════════════════════════════════════════════
            STOP_WORDS_VOIX = [
                # Stops directs
                "tais-toi", "tais toi", "silence", "stop", "stoppe", "stoppez",
                "arrête", "arrete", "arrêtes", "arrête-toi", "arrete toi",
                # Argot / familier
                "ta gueule", "tagueule", "la ferme", "ferme-la", "ferme la", "chut",
                "boucle-la", "boucle la", "bouclela",
                # Formulations
                "je t'arrête", "je t arrete", "je t'interromps",
                "assez", "basta", "ça suffit", "ca suffit", "c'est bon", "ok stop",
                "ok arrête", "ok arrete", "non arrête", "non arrete",
                "tu peux te taire", "tu peux arrêter", "arrête de parler"
            ]

            is_stop_command = any(w in texte for w in STOP_WORDS_VOIX)

            # ── 1. INTERRUPTION VOCALE IMMÉDIATE (STOP / TAIS-TOI) ───────
            if is_stop_command:
                STOP_PARLER = True
                print(f"[JARVIS] ✋ Arrêt vocal immédiat : '{texte}'")
                try:
                    if pygame and pygame.mixer.get_init():
                        pygame.mixer.music.stop()
                except Exception:
                    pass
                send_web_broadcast_sync({"action": "stop_audio"})
                send_web_broadcast_sync({"action": "set_state", "state": "idle"})
                continue

            # ── 2. FILTRE D'ÉCHO (si JARVIS parle, éviter qu'il s'entende lui-même) ───
            if is_speaking:
                is_echo = False
                if _last_jarvis_speech and len(texte.split()) >= 2:
                    mots_capturés  = set(texte.lower().split())
                    mots_jarvis    = set(_last_jarvis_speech.lower().split())
                    mots_communs   = mots_capturés & mots_jarvis
                    if len(mots_capturés) > 0 and len(mots_communs) / len(mots_capturés) > 0.45:
                        is_echo = True
                        print(f"[JARVIS] Écho ignoré : '{texte[:60]}'")

                if is_echo:
                    continue  # C'est l'écho de JARVIS dans les haut-parleurs → on ignore

            # ── 3. MOTS-CLÉS DE VEILLE (mode normal) ────────────────────
            SLEEP_WORDS = ["merci", "ce sera tout", "repos", "au revoir"]
            if not ALWAYS_LISTEN_MODE and any(word in texte for word in SLEEP_WORDS):
                if jarvis_actif:
                    jarvis_actif = False
                    executer_async_safe(parler("A votre service Mickael. Je me mets en veille."))
                continue

            # ── 4. MODE ÉCOUTE PERMANENTE (Bypass mot-clé + Barge-in naturel) ─
            if ALWAYS_LISTEN_MODE:
                if is_speaking:
                    STOP_PARLER = True
                    try:
                ...
                if commande:
                    print(f"[ALWAYS LISTEN] 🎙 Traitement non-bloquant : '{commande}'")
                    executer_async_safe(_traiter_commande_vocale_complete(commande))
                continue  # Retourne écouter immédiatement sans attendre !

            # ── 5. MODE PAR DÉFAUT (Mot-clé 'Jarvis') ────────────────────
            ...
            if wake_word_detected or jarvis_actif:
                ...
                if commande:
                    ...
                    if wake_word_detected:  # "Jarvis" prononcé seul
                        executer_async_safe(parler("Oui Mickael, je vous écoute."))
            else:
                pass
...
def _version_tuple_compare(v_str):
    try:
        import re
...
def verifier_mises_a_jour():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            v_remote = _version_tuple_compare(remote_version)
            v_local  = _version_tuple_compare(CURRENT_VERSION)
            
            # Comparaison numérique de version (ex: 10.0 est bien supérieur à 9.0)
            if v_remote > v_local:
                ...
                print(f"[UPDATE] Systeme a jour (v{CURRENT_VERSION} >= v{remote_version})")
...
def start_ia():
    ...
    async def start_ws():
        ...
        async with websockets.serve(ws_handler, "0.0.0.0", 8765, max_size=52428800, ping_interval=30, ping_timeout=30):
...
def start_mobile_http_server():
    ...
    class MobileHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def end_headers(self):
            # Désactive le cache navigateur pour forcer le rechargement des fichiers mis à jour
...
def vider_cache_webview_si_nouvelle_version():
    ...
    # Sous-dossiers à supprimer (cache pur, pas les données christopher critiques)
    cache_folders = [
        "Cache",
        "Code Cache",
        "Service Worker",
...
def main():
    ...
    def _servir_dist_python(port=5173):
        ...
        class DistHandler(http.server.SimpleHTTPRequestHandler):
            ...
            def log_message(self, *a):
                pass  # silencieux
        with socketserver.TCPServer(("", port), DistHandler) as httpd:
    ...
    # Surveillance automatique des nouveaux emails en direct (toutes les 45s)
    def _surveillance_emails_daemon():
        from google_services import verifier_nouveaux_emails, gmail_est_configure
        time.sleep(12)  # Attendre que JARVIS soit entièrement démarré
        while True:
            try:
                if gmail_est_configure():
                    res = verifier_nouveaux_emails()
                    if res.get("configured"):
                        ...
                        nouveaux = res.get("new_emails", [])
                        if nouveaux and not res.get("first_run"):
                            for ne in nouveaux:
                                vocal_alert = f"Mickael, vous avez reçu un nouvel email de {ne['sender']} avec pour objet : {ne['subject']}."
                                send_web_broadcast_sync({
                                ...
                                if not is_speaking:
                                    executer_async_safe(parler(vocal_alert))
            except Exception as _e_em:
                print(f"[SURVEILLANCE EMAIL] Erreur : {_e_em}")
            time.sleep(45)

    threading.Thread(target=_surveillance_emails_daemon, daemon=True).start()
