...
async def jarvis_vision_camera(question_christopher=None):
    """Capture une image depuis la caméra (via le frontend ou OpenCV) et l'analyse avec Gemini Vision."""
    import builtins
    USER_NAME = builtins.get_user_name() if hasattr(builtins, "get_user_name") else "Mickael"
    ...
    prompt_cam = f"{USER_NAME} te montre une image via sa caméra. Sa demande : '{question_christopher or 'Décris ce que tu vois'}'. Analyse l'image et réponds précisément."
...
async def jarvis_vision_navigateur(question_christopher=None):
    ...
    if getattr(builtins, "WEBCAM_ACTIVE", False):
        ...
        return await jarvis_vision_camera(question_christopher)
    ...
    try:
        ...
        if question_christopher:
            prompt_vision = f"{USER_NAME} te montre son navigateur/écran. Sa demande : '{question_christopher}'. Analyse l'image et réponds précisément."
        else:
