...
def create_plan(goal: str, context: str = "") -> dict:
    ...
    try:
        text = gemini_generate(user_input, model=PLANNER_MODEL, system_instruction=PLANNER_PROMPT).strip()
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        text = gemini_generate(prompt, model=PLANNER_MODEL, system_instruction=PLANNER_PROMPT).strip()
        text = _extract_json(text)
        plan = json.loads(text)
