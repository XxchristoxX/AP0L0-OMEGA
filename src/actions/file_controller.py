# src/actions/file_controller.py
# ============================================================================
# FILE CONTROLLER - VERSIÓN FUSIONADA CON MEJORAS
# ============================================================================
import os
import shutil
import platform
...
# ===== NUEVAS FUNCIONES =====
def _find_duplicates(path: str) -> str:
    ...
    result = f"Found {len(duplicates)} duplicate(s):\n" + "\n".join(str(d) for d in duplicates[:20])
    return result

def _compress_folder(path: str, name: str = "") -> str:
    target = _resolve_path(path)
    if not target.exists() or not target.is_dir():
        return f"Path is not a directory: {path}"
    zip_name = name or target.name + ".zip"
...
def _extract_archive(path: str, destination: str = "") -> str:
    target = _resolve_path(path)
    if not target.exists():
        return f"File not found: {path}"
    dest = _resolve_path(destination) if destination else target.parent / target.stem
...
def delete_file(path: str, name: str = "") -> str:
    try:
        ...
        protected = {_get_desktop(), _get_downloads(), _get_documents(), _get_pictures(), _get_music(), _get_videos(), Path.home()}
...
def file_controller(parameters: dict = None, response=None, player=None, session_memory=None) -> str:
    ...
    try:
        if action == "list":
            ...
            return _find_duplicates(path)
        elif action == "compress":
            return _compress_folder(path, name)
        elif action == "extract":
            return _extract_archive(path, params.get("destination", ""))
        else:
