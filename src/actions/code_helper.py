# src/actions/code_helper.py
# ============================================================================
# CODE HELPER - VERSIÓN FUSIONADA CON IA Y AST
# ============================================================================
import subprocess
import sys
import json
...
from src.core.config import LITE_MODEL
...
# ===== NUEVA FUNCIÓN: ANÁLISIS AST =====
def _analyze_ast(code: str) -> dict:
    """Analiza el código Python con AST y devuelve estadísticas."""
    if not HAS_AST:
...
# ===== NUEVA FUNCIÓN: DETECCIÓN DE INTENCIÓN MEJORADA =====
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    test_kw = ["test", "unit test", "prueba", "testear"]
    if any(k in desc for k in test_kw) and (code or file_path):
        return "test"

    security_kw = ["security", "secure", "güvenlik", "vulnerabilidad"]
    if any(k in desc for k in security_kw) and (code or file_path):
...
# ===== NUEVA FUNCIÓN: GENERAR PRUEBAS UNITARIAS =====
def _generate_tests(code: str, language: str) -> str:
    prompt = f"Genera pruebas unitarias para el siguiente código en {language}:\n\n{code[:4000]}"
    model = _get_gemini()
    response = model.generate_content(prompt)
    return _clean_code(response.text) if response else ""

# ===== NUEVA FUNCIÓN: REVISIÓN DE SEGURIDAD =====
def _security_review(code: str) -> str:
    prompt = f"Revisa el siguiente código en busca de vulnerabilidades de seguridad:\n\n{code[:4000]}"
    model = _get_gemini()
    response = model.generate_content(prompt)
...
def _run_file(path: Path, args: list, timeout: int) -> str:
    ...
    try:
        result = subprocess.run(interp + [str(path)] + (args or []), capture_output=True, text=True,
                                encoding="utf-8", errors="replace", timeout=timeout, cwd=str(path.parent))
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        ...
        if not _has_error(last_output):
            msg = f"Build complete, sir. The code is working after {attempt} attempt{'s' if attempt > 1 else ''}. Saved to {path}."
            if speak: speak(msg)
            return f"{msg}\n\nOutput:\n{last_output}"
        ...
        try:
            ...
            if speak: speak(msg)
            return msg
    msg = f"I was unable to build a working version after {MAX_BUILD_ATTEMPTS} attempts, sir. The last error was: {last_output[:200]}"
    if speak: speak(msg)
    return f"{msg}\n\nLast code saved to: {path}"

# ===== ACCIONES CON NUEVAS CAPACIDADES =====
def _write_action(description, language, output_path, player) -> str:
    ...
    try:
        ...
        print(f"[Code] ✅ Written: {path}")
        # Análisis AST opcional
        if language.lower() == "python":
            analysis = _analyze_ast(code)
            if "error" not in analysis:
                print(f"[Code] AST: {analysis}")
...
Optimized code:"""
    ...
    return f"Code optimized. {status}\nLines: {original_lines} → {optimized_lines} ({'−' if diff > 0 else '+'}{abs(diff)} lines)\n\nPreview:\n{_preview(optimized)}"
...
def _test_action(file_path, code, language, player) -> str:
    if file_path and not code:
        ...
        if err:
            return err
    if not code:
        return "Please provide code or a file path to test, sir."
    if player:
        player.write_log("[Code] Generating tests...")
    tests = _generate_tests(code, language or "python")
    if not tests:
        return "Could not generate tests."
    path = _resolve_save_path("", "python")
    save_path = path.parent / f"test_{path.stem}.py"
    _save_file(save_path, tests)
    return f"Tests generated and saved to: {save_path}\n\n{tests[:500]}..."

# ===== NUEVA ACCIÓN: SECURITY REVIEW =====
def _security_review_action(file_path, code, player) -> str:
    if file_path and not code:
        code, err = _read_file(file_path)
        if err:
            return err
    ...
    if player:
        player.write_log("[Code] Security review...")
    review = _security_review(code)
    if not review:
        return "Could not perform security review."
    return f"Security Review:\n{review}"

# ===== ACCIÓN PRINCIPAL (FUSIONADA) =====
def code_helper(
    parameters: dict,
    response=None,
    player=None,
    ...
    if action == "write":
        ...
        return _security_review_action(file_path, code, player)
    elif action == "screen_debug":
        return _screen_debug_action(description, file_path, player, speak)
    else:
        return f"Unknown action: '{action}'. Use write, edit, explain, run, build, optimize, test, security_review, screen_debug."