# src/actions/browser_control.py
# ============================================================================
# BROWSER CONTROL - VERSIÓN FUSIONADA CON IA
# ============================================================================
...
def _call_gemini(prompt: str, max_tokens: int = 500) -> str:
    """Usa Gemini para interpretar la pantalla o generar acciones."""
    try:
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if p and current.get("Default") == "1":
        is_rel = current.get("IsRelative", "1") == "1"
        default_path = str(base / p) if is_rel else p
    return default_path if default_path and Path(default_path).exists() else None
...
def _detect_default_browser() -> str:
    try:
        if _OS == "Windows":
            ...
            out = subprocess.run(["defaults", "read", "com.apple.LaunchServices/com.apple.launchservices.secure", "LSHandlers"], capture_output=True, text=True, timeout=5).stdout.lower()
            for kw in ("firefox", "opera", "brave", "vivaldi", "safari", "chrome", "edge"):
                if kw in out:
                    return kw
        else:
            out = subprocess.run(["xdg-settings", "get", "default-web-browser"], capture_output=True, text=True, timeout=5).stdout.lower()
...
def _open_native(url: str, browser_name: Optional[str]) -> str:
    ...
    if name:
        ...
        if exe:
            try:
                subprocess.Popen([exe, url] if url else [exe], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                return f"Opened in {name}: {url}" if url else f"Opened {name}."
            except Exception as e:
...
# ===== CLASE _BrowserSession (CON MEJORAS) =====
class _BrowserSession:
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            return
        self._thread = threading.Thread(target=self._run_loop, daemon=True, name=f"BrowserThread-{self.browser_name}")
        self._thread.start()
    ...
    async def _launch(self):
        ...
        if self._spec is None:
            raise RuntimeError(f"'{self.browser_name}' bu platformda ({_OS}) desteklenmiyor.")
        ...
        if engine_name == "firefox":
            ...
            kwargs: dict = {"headless": False, "slow_mo": 0, "viewport": None, "no_viewport": True, "timeout": 25000}
            if exe:
        ...
        if engine_name == "webkit":
            ...
            kwargs = {"headless": False, "slow_mo": 0, "viewport": None, "no_viewport": True, "timeout": 25000}
        ...
        kwargs = {
            "headless": False, "slow_mo": 0, "viewport": None, "no_viewport": True, "timeout": 25000,
        ...
        try:
            ...
            print(f"[Browser] ⚠️  Real profile failed for {self.browser_name}: {e}")
    ...
    async def go_to(self, url: str) -> str:
        ...
        try:
            ...
            if prev_url in ("about:blank", "", None):
                try:
    ...
    async def click(self, selector: str = None, text: str = None, description: str = None) -> str:
        ...
        try:
            if description:
                # Usar Gemini como ayuda (fallback a texto si falla)
                try:
                    ...
                    pass
                try:
                    await page.get_by_role("button", name=description).first.click(timeout=5000)
                    return f"Clicked button: '{description}'"
                except:
                    pass
                try:
                    await page.locator(f"*:has-text('{description}')").first.click(timeout=5000)
                    return f"Clicked element containing: '{description}'"
                except:
    ...
    async def type_text(self, selector: str = None, text: str = "", clear_first: bool = True, description: str = None) -> str:
        page = await self._get_page()
        try:
            if description:
                try:
                    ...
                    if await el.count() == 0:
                        el = page.get_by_label(description, exact=False).first
                except:
                    el = page.locator("input, textarea, [contenteditable]").first
            elif selector:
            ...
            return f"Typed: {text[:50]}..." if text else "Typed (empty)."
        except Exception as e:
    ...
    # ----- NUEVOS MÉTODOS CON IA (AÑADIDOS) -----
    async def smart_click(self, description: str) -> str:
        """Usa IA para encontrar y hacer clic en un elemento descrito en lenguaje natural."""
        return await self.click(description=description)

    async def smart_type(self, description: str, text: str) -> str:
        """Usa IA para encontrar un campo de entrada y escribir texto."""
        return await self.type_text(description=description, text=text)

    async def summarize_page(self) -> str:
        """Resume el contenido de la página actual usando Gemini."""
        page = await self._get_page()
        try:
            text = await page.inner_text("body")
            if len(text) > 8000:
                text = text[:8000]
            prompt = f"Resume el siguiente contenido de página web en 5 puntos clave:\n\n{text}"
            summary = _call_gemini(prompt, max_tokens=300)
            return f"Resumen de la página:\n{summary}"
        except Exception as e:
            return f"Error al resumir: {e}"

    async def analyze_screenshot(self, question: str) -> str:
        """Toma una captura de pantalla y la analiza con Gemini."""
        page = await self._get_page()
        try:
            screenshot = await page.screenshot(type="png", full_page=False)
            import base64
            img_b64 = base64.b64encode(screenshot).decode()
            prompt = f"Analiza esta captura de pantalla y responde: {question}"
            client = genai.Client(api_key=_get_api_key())
            ...
            return response.text.strip() if response and response.text else "No se pudo analizar."
        except Exception as e:
...
def browser_control(parameters: dict = None, response=None, player=None, session_memory=None) -> str:
    ...
    # Acciones de gestión de sesiones
    if action in ("switch", "list_browsers", "close_all", "close"):
        if action == "switch":
    ...
    # Navegación nativa o automática
    if action in ("go_to", "search", "new_tab"):
        ...
        if action == "search":
            base = _SEARCH_ENGINES.get(params.get("engine", "google").lower(), _SEARCH_ENGINES["google"])
    ...
    # Acciones interactivas (requieren sesión automática)
    try:
        sess = _registry.get(browser)
    except Exception as e:
    ...
    try:
        ...
        if action == "click":
            result = sess.run(sess.click(params.get("selector"), params.get("text"), params.get("description")))
        elif action == "type":
            result = sess.run(sess.type_text(params.get("selector"), params.get("text", ""), params.get("clear_first", True), params.get("description")))
        elif action == "scroll":
            ...
            result = sess.run(sess.get_text(params.get("selector", "body")))
        elif action == "get_url":
            ...
            result = sess.run(sess.summarize_page())
        elif action == "analyze_screenshot":
            question = params.get("question", "¿Qué ves en esta página?")
            result = sess.run(sess.analyze_screenshot(question))
        else:
