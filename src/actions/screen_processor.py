# src/actions/screen_processor.py
"""
Captura de pantalla y cámara con vista en vivo para AP0L0.
"""
...
if not _CONFIG_PATH.exists():
    _CONFIG_PATH = _BASE / "src" / "config" / "api_keys.json"
...
_SYSTEM_PROMPT = (
    ...
    "For technical questions give specific, expert answers. "
...
def _capture_screen() -> tuple[bytes, str]:
    """Captura la pantalla completa y devuelve bytes."""
    if not _MSS:
...
def _capture_camera() -> tuple[bytes, str]:
    """Captura un frame de la cámara y devuelve bytes."""
    if not _CV2:
...
# ===== NUEVA FUNCIÓN: STREAM EN VIVO DE LA CÁMARA =====
def start_camera_stream(callback, camera_index: int = 0):
    """Inicia un stream en vivo de la cámara que llama al callback con cada frame."""
    if not _CV2:
...
# ===== CLASE _VisionSession (mantener igual) =====
class _VisionSession:
    ...
    async def _session_loop(self) -> None:
        if not _GENAI:
            raise RuntimeError("google-genai no está instalado; visión cloud desactivada.")
    ...
    async def _play_loop(self) -> None:
        if sd is None:
            # La respuesta visual puede seguir procesándose aunque no exista
            # salida de audio local; no hacer fallar toda la sesión por ello.
            while True:
        ...
        if angle == "camera":
            ...
            if player and hasattr(player, "start_camera_stream"):
                try:
                    ...
                    print(f"[Vision] ⚠️ Camera stream failed: {e}")
        else:
