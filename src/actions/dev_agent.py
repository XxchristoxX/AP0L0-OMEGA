# src/actions/dev_agent.py
# ============================================================================
# DEV AGENT - VERSIÓN FUSIONADA CON PLANIFICACIÓN AVANZADA
# ============================================================================
import subprocess
import sys
import json
...
from src.core.config import PLANNER_MODEL, LITE_MODEL
...
7. For Python packages, include __init__.py files in subdirectories."""
    try:
...
Code for {file_path}:"""
    try:
        ...
        # Si es un directorio y no existe __init__.py, crearlo para Python
        if language.lower() == "python" and full_path.parent != project_dir:
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    ...
    try:
        ...
        result = subprocess.run(parts, capture_output=True, text=True, encoding="utf-8", errors="replace",
    ...
    for fix_path in files_to_fix:
        ...
        for fp, code in file_codes.items():
            if fp != fix_path and code:
                ...
                other_ctx += f"\n--- {fp} ---\n{snippet}\n"
        line_hint = f"\nError appears to be near line {error_line} in this file." if (error_line and fix_path == error_file) else ""
...
def _build_project(
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output, run_command):
            msg = f"Project '{proj_name}' is working, sir. Built in {attempt} attempt{'s' if attempt > 1 else ''}. Saved to: {project_dir}"
            if speak: speak(msg)
            return f"{msg}\n\nOutput:\n{last_output}"
        ...
        try:
            updated = _fix_files(last_output, description, files, file_codes, language, project_dir, entry_point)
    ...
    msg = f"I couldn't fully fix '{proj_name}' after {MAX_FIX_ATTEMPTS} attempts, sir. Project is saved at {project_dir} — open it in VSCode and check manually."
    if speak: speak(msg)
    return f"{msg}\n\nLast error:\n{last_output[:600]}"
...
def dev_agent(
    ...
    return _build_project(description, language, project_name, timeout, speak, player)