# src/actions/computer_control.py
# ============================================================================
# COMPUTER CONTROL - VERSIÓN FUSIONADA CON IA Y MEJORAS
# ============================================================================
import io
import json
import platform
...
def _call_gemini(prompt: str, max_tokens: int = 500) -> str:
    try:
        ...
        print(f"[ComputerControl] Gemini error: {e}")
        return ""

# ===== FUNCIONES DE UTILIDAD (CONSERVADAS Y MEJORADAS) =====
...
# ===== FUNCIONES DE ACCIONES (CONSERVADAS Y MEJORADAS) =====
...
def _focus_window(title: str) -> str:
    ...
    if os_name == "linux":
        try:
            ...
            pass
        try:
            result = subprocess.run(["xdotool", "search", "--name", title, "windowactivate"], capture_output=True, timeout=5)
            time.sleep(0.3)
            return f"Focused window: {title}"
        except FileNotFoundError:
...
# ===== NUEVA FUNCIÓN: SCREEN_FIND MEJORADA =====
...
# ===== NUEVA FUNCIÓN: SCREEN_CLICK CON IA =====
...
def computer_control(
    ...
    try:
        ...
        if action == "smart_type":
            return _smart_type(params.get("text", ""), params.get("clear_first", True))
