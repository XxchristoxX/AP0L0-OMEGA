# src/actions/upload_video.py
"""
Hardcoded automation: uploads the video sitting on the Desktop (named 'video')
to TikTok Studio, using robust visual search (locateOnScreen) with fallback
coordinates if images are not found.
...
RESOURCES_DIR   = BASE_DIR / "resources" / "upload_buttons"  # optional images for locateOnScreen
...
def _find_video_on_desktop() -> Path | None:
    """Busca un archivo cuyo nombre contenga 'video' (sin extensión) en el escritorio.
    Prioriza el más reciente."""
    desktop = Path.home() / "Desktop"
    candidates = []
    for f in desktop.iterdir():
        if f.is_file() and "video" in f.stem.lower():
...
def _click_center_of_screen():
    w, h = pyautogui.size()
    pyautogui.click(w // 2, h // 2)


def _click_image_or_fallback(image_name: str, fallback_x: int, fallback_y: int,
                             confidence: float = 0.8, region=None) -> bool:
    """Busca una imagen en RESOURCES_DIR y hace clic; si no la encuentra, usa coordenadas de respaldo."""
    img_path = RESOURCES_DIR / image_name
    if img_path.exists():
        try:
            pos = pyautogui.locateCenterOnScreen(str(img_path), confidence=confidence, region=region)
            if pos:
                pyautogui.click(pos)
                print(f"[UploadVideo] Clicked on image: {image_name}")
                return True
        except Exception as e:
            print(f"[UploadVideo] locateOnScreen failed for {image_name}: {e}")
    # Fallback: clic en coordenadas
    pyautogui.click(fallback_x, fallback_y)
    print(f"[UploadVideo] Fallback click at ({fallback_x}, {fallback_y})")
    return False
...
def _run_upload_flow(brief: str, caption_holder: dict,
    ...
    time.sleep(3.0)  # esperar a que la ventana esté activa

    # 2. Nueva pestaña y navegar a TikTok Studio
    pyautogui.hotkey("ctrl", "t")
    time.sleep(0.6)
    _paste("https://www.tiktok.com/tiktokstudio/upload")
    time.sleep(0.2)
    pyautogui.press("enter")
    time.sleep(5.0)  # carga de la página

    # 3. Clic en el centro de la pantalla (zona de drop)
    _click_center_of_screen()
    time.sleep(0.8)
    pyautogui.press("tab")
    time.sleep(0.3)
    pyautogui.press("enter")
    time.sleep(2.0)  # se abre el diálogo de archivos

    # 4. Buscar el archivo 'video' en el diálogo de archivos
    pyautogui.hotkey("ctrl", "f")
    time.sleep(0.6)
    pyautogui.write("video", interval=0.03)
    time.sleep(1.5)  # filtrar resultados

    # 5. Doble clic en el archivo encontrado (posición relativa)
    # Buscar la imagen del archivo o usar coordenada de respaldo
    # Intentamos buscar el nombre del archivo en la lista (simplificado: hacemos doble clic en una posición fija)
    # Mejor: buscar el archivo exacto en el escritorio y abrirlo con doble clic en la lista de resultados
    # Dado que no podemos localizar fácilmente, usamos posición relativa a la ventana del diálogo
    # En una pantalla 1920x1080 de referencia, el archivo suele aparecer alrededor de (240, 135)
    # Escalamos a la resolución actual
    file_x = int(w * 240 / 1920)
    file_y = int(h * 135 / 1080)
    # Intentar localizar el icono del archivo (si tenemos imagen de referencia)
    _click_image_or_fallback("file_icon.png", file_x, file_y, confidence=0.7)
    # Doble clic para abrir
    pyautogui.doubleClick()
    time.sleep(4.0)  # TikTok procesa el video; aparece el campo de caption

    # 6. Esperar a que el caption esté listo y pegarlo
    caption_ready.wait(timeout=15)
    caption = (caption_holder.get("caption") or brief).strip()
    # Navegar al campo de caption (dos tabs)
    pyautogui.press("tab")
    pyautogui.press("tab")
    time.sleep(0.3)
    ...
    # 7. Navegar al botón de publicación (17 tabs) y presionar Enter
    time.sleep(6)
    for _ in range(17):
        pyautogui.press("tab")
        time.sleep(0.12)
    ...
    pyautogui.press("enter")
    time.sleep(1)
    # Clic en el botón de confirmación (posible pop-up)
    # Intentar localizar el botón "Publicar" o "Post" con imagen
    _click_image_or_fallback("post_button.png", int(w * 1089 / 1920), int(h * 653 / 1080), confidence=0.7)
    _click_image_or_fallback("confirm_button.png", int(w * 1200 / 1920), int(h * 655 / 1080), confidence=0.7)
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    # Verificar que el archivo 'video' existe en el escritorio
    video_path = _find_video_on_desktop()
    if not video_path:
        return ("I couldn't find a video file named 'video' on your Desktop. "
                "Please ensure the file exists (any extension, name contains 'video').")

    print(f"[UploadVideo] Found video: {video_path}")

    # Kick off SEO caption generation in the BACKGROUND
    caption_holder: dict = {}
