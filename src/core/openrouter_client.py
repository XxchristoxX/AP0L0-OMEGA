...
def _get_base_dir() -> Path:
    ...
    return Path(__file__).resolve().parent.parent.parent  # Raíz del proyecto

BASE_DIR     = _get_base_dir()
API_KEY_PATH = BASE_DIR / "config" / "api_keys.json"
API_KEY_CANDIDATES = (
    BASE_DIR / "config" / "api_keys.json",
    BASE_DIR / "src" / "config" / "api_keys.json",
)
...
class OpenRouterClient:
    ...
    def _mark_rate_limited(self, model: str) -> None:
        ...
        logger.warning(f"[OpenRouter] Rate limited: {model} — cooling down for {RATE_LIMIT_COOLDOWN}s")
    ...
    def _call(
        ...
        if not self.enabled:
            logger.info("[OpenRouter] Sin API key; proveedor cloud desactivado.")
            return None
        payload: dict = {
            "model":       model,
            "messages":    messages,
        ...
        for attempt in range(1, MAX_RETRIES_PER_MODEL + 1):
            try:
                ...
                logger.warning(f"[OpenRouter] {model} → Timeout (attempt {attempt}/{MAX_RETRIES_PER_MODEL})")
