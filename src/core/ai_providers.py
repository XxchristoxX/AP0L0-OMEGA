...
from src.core.config import get_api_key, get_selected_text_model
...
def generate_gemini(
    ...
    if model is None:
        model = get_selected_text_model() or "gemini-3.5-flash-lite"
...
def generate(
    ...
    log.info("Generating with provider: %s (model: %s)", provider, model)
